...
import { DEPTH_OUTPUT_RESOLUTION, MAX_DEPTH_SECONDS, closestVideoDepthRatioLabel, fitVideoDepthOutputSize } from "@/lib/video-depth-size";
...
type WorkflowCanvasProps = {
  ...
  nodeDefaults?: WorkflowNodeDefaults;
  /**
...
async function uploadWorkflowFile(file: File, kind: Exclude<WorkflowUploadKind, "image">, workflowId: string, workflowNodeId: string, media?: { durationSeconds?: number; dimensions?: { width: number; height: number } }, onProgress?: (progress: number) => void) {
  ...
  formData.append("flow", "workflow");
  formData.append("workflowId", workflowId);
  formData.append("workflowNodeId", workflowNodeId);
  if (media?.durationSeconds) formData.append("durationSeconds", String(media.durationSeconds));
  if (media?.dimensions) formData.append("dimensions", JSON.stringify(media.dimensions));
  const data = await uploadWorkflowFormDataWithProgress<{ url?: string; error?: string; dedup?: boolean; name?: string; posterUrl?: string }>(getWorkflowUploadApiUrl("/api/upload-file"), formData, onProgress, token);
...
function validateWorkflowUploadNodeFile(file: File, kind: "image" | "video" | "audio" | "text", media?: { durationSeconds?: number; dimensions?: { width: number; height: number } }, text?: string) {
  ...
  if (kind === "video") {
    return validateMediaUploadFile(file, "video") ?? validateMediaUploadMetadata("video", { durationSeconds: media?.durationSeconds, width: media?.dimensions?.width, height: media?.dimensions?.height });
  }
  return validateMediaUploadFile(file, "audio") ?? validateMediaUploadMetadata("audio", { durationSeconds: media?.durationSeconds });
}

// 资产库不区分对话流/工作流上传：上传类一律显示在「上传的资产」。工作流上传的图片按其真实来源写
// workflow_upload_images（历史上这里硬写成 conversation_uploads，属分叉），读取端把 conversation_uploads /
// workflow_upload_images / workflow_uploads 统一当「上传图片」，所以老数据不用回填也照样显示。
function persistWorkflowUploadNodeAsset(input: { url: string; name: string; mediaType: "image" | "video" | "audio" | "document"; workflowId: string; workflowNodeId: string; sourcePrompt: string; file: File; dimensions?: { width: number; height: number }; durationSeconds?: number; settings?: { ratio?: string; resolution?: string; duration?: string }; contentHash?: string }) {
  return fetch("/api/media-assets", {
    ...
    body: JSON.stringify({
      url: input.url,
      name: input.name,
      currentCategory: input.mediaType === "image" ? "workflow_upload_images" : input.mediaType === "video" ? "workflow_upload_videos" : input.mediaType === "audio" ? "workflow_upload_audios" : "workflow_upload_documents",
...
// 「视频截图」产出的图片节点：走的是统一上传链路，但标题要显示「视频截图 xxx」而不是「上传图片 xxx」。
// 它在节点头部图标/参数标签/上传态判定上与上传节点同待遇，故统一用 isWorkflowUploadLikeTitle 判断。
const WORKFLOW_VIDEO_FRAME_NODE_TITLE = "视频截图";

function isWorkflowUploadLikeTitle(title: string | undefined) {
  return Boolean(title && (title.startsWith("上传") || title === WORKFLOW_VIDEO_FRAME_NODE_TITLE));
}

function getWorkflowNodeHeaderIcon(node: WorkflowNode) {
  if (node.kind === "text" && node.title === "上传文本") return RiTextSnippet;
  if (node.kind === "image" && (node.title === "上传图片" || node.title === WORKFLOW_VIDEO_FRAME_NODE_TITLE)) return RiImageLine;
...
function getWorkflowNodeParamParts(node: WorkflowNode): { modelLabel: string; ratio: string; resolution: string; duration: string; sizeText: string; note?: string } {
  ...
  // ⭐ 深度动作捕捉例外：它的输出档是写死的规格（唯一权威 video-depth-size.ts，短边 512、统一记 480p），
  //   ⛔ 不许由宽高反推 —— `getVideoResolutionFromDimensions(896,512)` 会落进 720p，
  //   于是节点头上显示 720p、而我们记账/发给上游的都是 480p，自己跟自己对不上。
  const resolutionText = isVideoDepthModel(node.data.model)
    ? (node.data.resolution ?? "")
    : realVideoDims ? (getVideoResolutionFromDimensions(realVideoDims.width, realVideoDims.height) ?? node.data.resolution ?? "") : (node.data.resolution ?? "");
...
// 在某视频模型支持的分辨率档里，挑总像素最接近实际视频尺寸的一档（视频快捷编辑贴源视频尺寸时用）。
// 与图片的 closestResolutionForImageDimensions 同思路，只是换成视频的档位与尺寸表。
function closestResolutionForVideoDimensions(model: string, ratio: string, dimensions?: { width: number; height: number }): string {
  const resolutions = getSupportedVideoResolutions(model);
  if (!resolutions.length) return "720p";
  if (!dimensions?.width || !dimensions.height) return normalizeVideoResolutionForModel(model, undefined);
  const targetPixels = dimensions.width * dimensions.height;
  let best = resolutions[0] as string;
  let bestDiff = Number.POSITIVE_INFINITY;
  for (const res of resolutions) {
    const expected = getExpectedVideoDimensions(model, res, ratio);
...
// 把真实秒数换成该模型支持的「N秒」档里最接近的一档（上传视频没有 duration 设置，只有真实 durationSeconds）。
function closestWorkflowVideoDurationLabel(model: string, durationSeconds?: number): string | undefined {
  const options = workflowVideoModels.find((item) => item.id === model)?.durations ?? [];
  if (!options.length) return undefined;
  if (!Number.isFinite(durationSeconds ?? Number.NaN) || !durationSeconds) return undefined;
  let best = options[0];
  let bestDiff = Number.POSITIVE_INFINITY;
  for (const option of options) {
    const seconds = Number(option.match(/(\d+(?:\.\d+)?)/)?.[1]);
    if (!Number.isFinite(seconds)) continue;
    const diff = Math.abs(seconds - durationSeconds);
    if (diff < bestDiff) { bestDiff = diff; best = option; }
  }
  return best;
}

function getCommonWorkflowRatioLabel(dimensions?: { width: number; height: number }) {  if (!dimensions?.width || !dimensions.height) return undefined;
  const ratio = dimensions.width / dimensions.height;
...
function pickWorkflowImageModel(preferred?: string): ModelName {
  if (preferred && frontendImageGenerationModels.some((model) => model.id === preferred)) return preferred as ModelName;
  if (frontendImageGenerationModels.some((model) => model.id === DEFAULT_WORKFLOW_IMAGE_MODEL)) return DEFAULT_WORKFLOW_IMAGE_MODEL;
  return (frontendImageGenerationModels[0]?.id ?? DEFAULT_IMAGE_MODEL) as ModelName;
}

function pickWorkflowVideoModel(preferred?: string): ModelName {
  if (preferred && workflowVideoModels.some((model) => model.id === preferred)) return preferred as ModelName;
  if (workflowVideoModels.some((model) => model.id === DEFAULT_WORKFLOW_VIDEO_MODEL)) return DEFAULT_WORKFLOW_VIDEO_MODEL;
  return (workflowVideoModels[0]?.id ?? DEFAULT_VIDEO_MODEL) as ModelName;
}

function getDefaultNodeData(kind: WorkflowNodeKind, prefs?: WorkflowNodeDefaults): WorkflowNodeData {
  if (kind === "text") return { text: "", prompt: "" };
  if (kind === "audio") return { prompt: "上传音频" };
  if (kind === "video") {
    const model = pickWorkflowVideoModel(prefs?.videoModel);
    const durationOptions = workflowVideoModels.find((item) => item.id === model)?.durations ?? fallbackVideoDurationOptions;
    const resolution = normalizeVideoResolutionForModel(model, prefs?.videoResolution ?? "720p");
    const ratio = prefs?.videoRatio === "智能比例" ? "智能比例" : normalizeVideoRatioForModel(model, prefs?.videoRatio ?? "16:9", resolution);
    const duration = prefs?.videoDuration && durationOptions.includes(prefs.videoDuration) ? prefs.videoDuration : (durationOptions.includes("8秒") ? "8秒" : durationOptions[0] ?? "5秒");
    return { model, ratio, resolution, duration, videoReferenceMode: "reference", prompt: "" };
  }
  const model = pickWorkflowImageModel(prefs?.imageModel);
  const resolution = normalizeImageResolutionForModel(model, prefs?.imageResolution ?? "2K");
  const supportedRatios = getSupportedImageRatios(model) as string[];
  const ratio = prefs?.imageRatio && prefs.imageRatio !== "智能比例" && supportedRatios.includes(prefs.imageRatio) ? prefs.imageRatio : (supportedRatios.includes("16:9") ? "16:9" : supportedRatios[0] ?? "16:9");
  return { model, ratio, resolution, prompt: "" };
}
...
type WorkflowVideoFrameKind = "first" | "last" | "current";

function getWorkflowVideoFrameLabel(frame: WorkflowVideoFrameKind) {
  return frame === "first" ? "首帧" : frame === "last" ? "尾帧" : "当前帧";
}

// 视频截帧唯一实现：右键菜单「导出首/尾/当前帧」（下载）与快捷菜单「视频截图」（进画布）共用，禁止再各写一份。
async function getWorkflowVideoFrameJpeg(node: WorkflowNode, frame: WorkflowVideoFrameKind) {
  ...
  return { blob, fileName: `${getWorkflowMediaBaseName(node, "workflow-video")}-${getWorkflowVideoFrameLabel(frame)}.jpg` };
}

async function exportWorkflowVideoFrame(node: WorkflowNode, frame: WorkflowVideoFrameKind) {
  const result = await getWorkflowVideoFrameJpeg(node, frame);
  if (!result) return;
    ...
    if (node.kind === "text") {
      ...
      return (
        <g>
          ...
          <text x={padding} y={padding + 18} fill={isPlaceholder ? "#a6a6a6" : "#222222"} fontFamily="PingFang SC, Microsoft YaHei UI, Microsoft YaHei, sans-serif" fontSize={16}>
...
function WorkflowSelectedNodeOverlay() {
  ...
  const title = node.kind === "text" ? (node.title === "上传文本" ? "上传文本（双击进入编辑模式）" : "文本输入（双击进入编辑模式）") : isWorkflowUploadLikeTitle(node.title) ? [node.title, getWorkflowUploadFileName(node)].filter(Boolean).join(" ") : [getNodeLabel(node.kind), mediaName].filter(Boolean).join(" ");
          ...
          <div className="flex h-[44px] items-center gap-0.5 rounded-[10px] bg-white px-1.5 shadow-[0_6px_24px_rgba(15,23,42,0.16)] ring-1 ring-black/5" style={{ fontSize: 14 }}>
            ...
            {!isVideoQuickMenuNode ? (
              ...
              <>
                ...
                >
                  <button type="button" className="flex h-[34px] items-center gap-1 whitespace-nowrap rounded-[8px] px-2 font-medium text-[#1f2937] hover:bg-gray-100">
                    <RiCameraLine className="h-[18px] w-[18px]" />
                    <span>视频截图</span>
                    <RiArrowDownSLine className="h-4 w-4 text-[#9ca3af]" />
                  </button>
                  {frameMenuOpen ? (
                    <div className="absolute left-1/2 top-full z-[10000] w-[150px] -translate-x-1/2 pt-1.5">
                      <div className="rounded-[10px] bg-white p-1 shadow-[0_10px_30px_rgba(15,23,42,0.18)] ring-1 ring-black/5">
                        {[{ frame: "first" as WorkflowVideoFrameKind, label: "截取首帧" }, { frame: "last" as WorkflowVideoFrameKind, label: "截取尾帧" }, { frame: "current" as WorkflowVideoFrameKind, label: "截取当前帧" }].map((item) => (
                          <button
                            key={item.frame}
                            type="button"
                            onClick={() => { setFrameMenuOpen(false); runtime.createVideoFrameImageNode(node, item.frame); }}
                            className="flex h-9 w-full items-center gap-2 whitespace-nowrap rounded-[8px] px-2 text-left text-[14px] text-[#1f2937] hover:bg-gray-100"
                          >
                            <RiImageLine className="h-[17px] w-[17px] text-[#6b7280]" />
...
function WorkflowCustomContextMenu({ menu, onClose, onAddNode, onUploadNode, onImportAsset, onUsePrompt, onConfirmDelete }: { menu: WorkflowContextMenuState; onClose: () => void; onAddNode: (kind: WorkflowNodeKind, pagePoint?: { x: number; y: number }) => void; onUploadNode: () => void; onImportAsset?: () => void; onUsePrompt: (node: WorkflowNode) => void; onConfirmDelete: (kindLabel: string, doDelete: () => void) => void }) {
  ...
  const isUploadedMediaNode = isWorkflowUploadLikeTitle(selectedWorkflowNode?.title);
...
export function WorkflowCanvas({ workflowId, value, onChange, workflowTitle, onCredit, onGeneratedMedia, onPreviewMedia, onShowTip, onUploadedAsset, getImageDisplayUrl, getVideoPosterDisplayUrl, enabledTextModelIds, textModelProviders = {}, enabledImageModelIds, enabledVideoModelIds, membershipTier = "free", membershipSettings, uploadRuleOverrides, promptLengthOverrides, creditRate, editModelToggles, leftSidebarVisible = true, leftSidebarToggleLabel, onToggleLeftSidebar, workflowAssets = [], referenceAssets = [], referenceAssetsLoadStatus = "idle", referenceAssetCounts, onLoadReferenceAssets, onLoadReferenceFilter, referenceFilterLoading, referenceFilterNextOffset, onLoadMoreReferenceAssets, onExternalFilesDrop, onOpenAssetImport, assetsToImport, onAssetsImported, nodeDefaults }: WorkflowCanvasProps) {
  ...
  const modelOptions = useMemo<WorkflowModelOptions>(() => ({ textModels, textModelProviders, imageModels, videoModels, creditRate, membershipTier, membershipSettings }), [imageModels, textModelProviders, textModels, videoModels, creditRate, membershipTier, membershipSettings]);
  ...
  const addNode = useCallback((kind: WorkflowNodeKind, pagePoint?: { x: number; y: number }) => {
    ...
    const draftNode: WorkflowNode = { id: createId("workflow_node"), kind, title: getNodeLabel(kind), x: 0, y: 0, data: defaultNodeData(kind) };
  ...
  const addNodeFromPrompt = useCallback((sourceNode: WorkflowNode) => {
    ...
    const defaultData = defaultNodeData(kind);
  ...
  }, [defaultNodeData, updateState, workflowId]);

  // targetNodeId：上传成为某节点的输入 → 放在它左侧并自动连线。
  // rightOfNode：上传结果作为某节点的「产物」（如视频截图）→ 放在它右侧、不连线。
  const addUploadedNode = useCallback((draftNode: Omit<WorkflowNode, "x" | "y">, targetNodeId?: string, rightOfNode?: WorkflowNode) => {
    const editor = editorRef.current;
    const current = stateRef.current;
    const viewport = editor?.getViewportPageBounds();
    ...
    const anchor = current.nodes.find((node) => node.id === anchorId);
    const fallback = viewport ? { x: viewport.x + viewport.w / 2 - size.w / 2, y: viewport.y + viewport.h / 2 - size.h / 2 } : undefined;
    const rightAnchor = rightOfNode ? current.nodes.find((node) => node.id === rightOfNode.id) ?? rightOfNode : undefined;
    const position = rightAnchor
      ? findNonOverlappingNodePosition(current.nodes, size, rightAnchor)
      : target ? findNonOverlappingNodePositionLeftOfTarget(current.nodes, size, target) : findNonOverlappingNodePosition(current.nodes, size, anchor, fallback);
  ...
  // imageTitle：图片节点的标题/提示词文案，默认「上传图片」；「视频截图」复用本上传链路时传入自己的文案。
  const handleUploadNodeFile = useCallback(async (file: File, targetNodeId?: string, onDuplicateTip?: (message: string) => void, rightOfNode?: WorkflowNode, imageTitle = "上传图片") => {
    try {
      if (file.type.startsWith("image/")) {
        ...
        addUploadedNode({ id: nodeId, kind: "image", title: imageTitle, data: { ...defaultNodeData("image"), prompt: imageTitle, imageDimensions: {}, ratio: normalizeWorkflowImageRatio(undefined, dimensions), uploadProgress: 1, uploadPreviewUrl: previewUrl } }, targetNodeId, rightOfNode);
        ...
        const data: WorkflowNodeData = { ...defaultNodeData("image"), prompt: imageTitle, images: [url], imageDimensions: { [url]: dimensions }, mediaSystemNames: { [url]: mediaName }, ratio: normalizeWorkflowImageRatio(undefined, dimensions), visualSize: undefined, uploadProgress: undefined, uploadPreviewUrl: undefined };
        updateNode(nodeId, data);
        URL.revokeObjectURL(previewUrl);
        // 视频截图和普通上传图片同待遇：都走上传归类，显示在资产库「上传的资产 · 上传图片」。
        // 入库成功后通知父级刷新资产库对应分类，否则右侧列表要手动刷新页面才出现（左侧计数走服务端所以先变了）。
        void persistWorkflowUploadNodeAsset({ url, name: mediaName, mediaType: "image", workflowId, workflowNodeId: nodeId, sourcePrompt: imageTitle, file, dimensions, settings: { ratio: data.ratio, resolution: data.resolution }, contentHash: uploaded.contentHash })
          .then(() => onUploadedAsset?.({ mediaType: "image" }))
          .catch((error) => console.warn("[media-assets] failed to persist workflow uploaded image", error));
        return nodeId;
      }
      if (file.type.startsWith("video/")) {
        ...
        if (validationError) return onShowTip?.(validationError);
        const defaultData = defaultNodeData("video");
        const nodeId = createId("workflow_node");
        const previewUrl = URL.createObjectURL(file);
        ...
        const data: WorkflowNodeData = { ...defaultData, prompt: "上传视频", videoUrl: url, posterUrl: uploadedVideo.posterUrl, videoDimensions: media.dimensions, durationSeconds: media.durationSeconds, videoCurrentTime: 0, mediaSystemNames: { [url]: mediaName }, ratio: media.dimensions ? getCommonWorkflowRatioLabel(media.dimensions) ?? defaultData.ratio : defaultData.ratio, visualSize: undefined, uploadProgress: undefined, uploadPreviewUrl: undefined };
        updateNode(nodeId, data);
        URL.revokeObjectURL(previewUrl);
        onUploadedAsset?.({ mediaType: "video" });
        return nodeId;
      }
      if (file.type.startsWith("audio/") || ["mp3", "wav"].includes(getWorkflowFileExtension(file))) {
        ...
        if (validationError) return onShowTip?.(validationError);
        const nodeId = createId("workflow_node");
        addUploadedNode({ id: nodeId, kind: "audio", title: "上传音频", data: { ...defaultNodeData("audio"), durationSeconds: media.durationSeconds, uploadProgress: 1 } }, targetNodeId);
        ...
        onUploadedAsset?.({ mediaType: "audio" });
        return nodeId;
      }
      if (file.type === "text/plain" || getWorkflowFileExtension(file) === "txt") {
        const nodeId = createId("workflow_node");
        addUploadedNode({ id: nodeId, kind: "text", title: "上传文本", data: { ...defaultNodeData("text"), uploadProgress: 1 } }, targetNodeId);
        ...
        const uploadedDocument = await uploadWorkflowFile(file, "document", workflowId, nodeId);
  ...
  }, [addUploadedNode, defaultNodeData, onShowTip, onUploadedAsset, updateNode, workflowId]);
  ...
  const createVideoFrameImageNode = useCallback(async (sourceNode: WorkflowNode, frame: WorkflowVideoFrameKind) => {
    try {
      const result = await getWorkflowVideoFrameJpeg(sourceNode, frame);
      if (!result) return;
      const file = new File([result.blob], result.fileName, { type: "image/jpeg" });
      const nodeId = await handleUploadNodeFile(file, undefined, undefined, sourceNode, WORKFLOW_VIDEO_FRAME_NODE_TITLE);
      if (nodeId) selectAndFocusUploadedNodes([nodeId], false);
    } catch (error) {
      onShowTip?.(toUserErrorMessage(error, "视频截图失败，请稍后重试。"));
    }
  }, [handleUploadNodeFile, onShowTip, selectAndFocusUploadedNodes]);
  ...
  const runImageNode = useCallback(async (node: WorkflowNode) => {
    ...
    try {
      ...
      // 参考图尺寸发送前拦截（与对话流、服务端共用 image-reference-image-rules 的唯一规则）：
      // 不合规的图会在生成阶段被上游拒（如 Recraft 要求单边 256–4096px），以前只会显示成"服务器繁忙"。
      // 受约束的模型集合 + 边长区间由 imageModelEnforcesReferenceImageSizeRules 唯一判定。
      if (imageModelEnforcesReferenceImageSizeRules(model) && referenceImages.length > 0) {
        const referenceImageNameByUrl = new Map<string, string>();
        const referenceImageDimByUrl = new Map<string, { width: number; height: number }>();
        for (const source of getIncomingNodes(node.id)) {
          for (const url of source.data.images ?? []) {
            if (url && source.data.mediaSystemNames?.[url]) referenceImageNameByUrl.set(url, source.data.mediaSystemNames[url]);
            const dim = source.data.imageDimensions?.[url];
            if (url && dim?.width && dim.height) referenceImageDimByUrl.set(url, { width: dim.width, height: dim.height });
          }
        }
        const referenceImageSizeError = await validateImageReferenceImagesBeforeSend(
          model,
          referenceImages.map((url) => {
            const dim = referenceImageDimByUrl.get(url);
            return { name: referenceImageNameByUrl.get(url), url, width: dim?.width, height: dim?.height };
          }),
          (url) => getStaticMediaUrl(url) ?? url,
        );
        if (referenceImageSizeError) {
          onShowTip?.(referenceImageSizeError);
          return updateNode(node.id, { isRunning: false, error: referenceImageSizeError, imageRequestId: undefined });
        }
      }
  ...
  }, [generateImageForNode, getEnabledImageModel, getInputText, getPromptReferenceUrls, getReferenceImages, getIncomingNodes, onShowTip, updateNode, uploadRuleOverrides, promptLengthOverrides]);
  ...
  const createImageEditNode = useCallback((sourceNode: WorkflowNode, options: { prompt: string; model?: ModelName; modelCandidates?: ModelName[]; highDef?: boolean; ratio?: string; resolution?: string; transparent?: boolean; bgRemove?: boolean; matchSourceImage?: boolean; ratioFromSourceImage?: boolean; position?: { x: number; y: number }; referenceImageOverride?: string; select?: boolean }) => {
    ...
    return node;
  }, [defaultNodeData, generateImageForNode, getEnabledImageModel, imageModels, updateNode, updateState]);
  ...
  // options 为空时行为与改造前完全一致（单模型、参考素材全部从连线/@提及推导）。
  // options 用于「视频快捷编辑」：注入源视频当参考视频 + 强制融合模式 + 模型候选链（依次尝试，前一个失败换下一个）。
  const runVideoNode = useCallback(async (node: WorkflowNode, options?: {
    modelCandidates?: ModelName[];
    referenceVideosOverride?: string[];
    referenceVideoDurations?: Record<string, number>;
    forceReferenceMode?: WorkflowVideoReferenceMode;
  }) => {
    const upstreamPrompt = getInputText(node.id);
    const ownPrompt = node.data.prompt?.trim() ?? "";
    const prompt = [ownPrompt, upstreamPrompt].filter(Boolean).join("\n\n").trim();
    ...
    for (const attemptModel of attemptModels) {
      ...
      // 候选链换模型时同步节点显示的 model/比例/分辨率（等待卡标签跟随实际尝试的模型），与图片编辑候选链一致。
      ...
      try {
        ...
        await pollVideoNode({ ...node, data: { ...node.data, model } }, taskId, prompt, model, settings, requestId, createData.usage);
        return;
      } catch (error) {
        lastError = error;
      }
  ...
  // 为严格保住「和源视频相同参数」，候选链只保留原生支持该分辨率的模型（1080p 只有 2.0/2.5 支持、4K 只有 2.0，
  // 否则 normalizeVideoResolutionForModel 会把 Mini/Fast 悄悄降到 720p，就不是"相同参数"了）。
  // 计费：每次尝试都是一次独立的 workflow_video_generation 任务（自带新 requestId），失败不计费，
  // 与图片编辑候选链一致，不会重复扣分。
  const createVideoEditNode = useCallback((sourceNode: WorkflowNode, options: { prompt: string; position?: { x: number; y: number }; select?: boolean }) => {
    const sourceUrl = sourceNode.data.videoUrl;
    if (!sourceUrl) return;
    const prompt = options.prompt.trim();
    if (!prompt) return;
    const current = stateRef.current;
    // 参数一律以「源视频的真实尺寸/真实时长」为准（有 videoDimensions/durationSeconds 就用它推），
    // 推不出来才退回节点上的设置。这样生成视频与上传视频都对：
    // - 生成视频：真实尺寸/时长本来就等于它的设置，推出来结果一致。
    // - 上传视频：节点上的 resolution/duration 是建节点时的默认值（720p / 8秒），不是真实值，必须靠真实尺寸推，
    //   否则上传一个 1920×1080 / 12 秒的视频会被错当成 720p / 8 秒。
    ...
    // 比例：模型没有完全一致的档时取最接近的一档（closestWorkflowRatioLabel 一定返回一个）。
    const ratio = sourceDims?.width && sourceDims.height ? closestWorkflowRatioLabel(sourceDims) : sourceNode.data.ratio;
    const chain = getVideoEditCandidates("video_quick", editModelToggles).filter((candidate) => videoModels.some((item) => item.id === candidate));
    if (chain.length === 0) {
      onShowTip?.("当前没有可用的视频模型");
      return;
    }
    // 分辨率优先：先按真实尺寸算出「该用哪一档」（以支持档位最全的 2.0 为基准算，避免被 Mini/Fast 的 720p 上限带偏），
    // 再据此过滤候选链——需要 4K 时只有 2.0、需要 1080p 时只剩 2.0/2.5；
    // 480p/720p 时 Mini → Fast → 2.0 → 2.5 都支持。
    const resolutionBaselineModel = chain.includes(DEFAULT_WORKFLOW_VIDEO_MODEL as ModelName) ? (DEFAULT_WORKFLOW_VIDEO_MODEL as ModelName) : chain[chain.length - 1];
    const resolution = sourceDims?.width && sourceDims.height
      ? closestResolutionForVideoDimensions(resolutionBaselineModel, ratio ?? "16:9", sourceDims)
      : sourceNode.data.resolution;
    const nativeChain = resolution ? chain.filter((candidate) => (getSupportedVideoResolutions(candidate) as readonly string[]).includes(resolution)) : chain;
    const modelCandidates = nativeChain.length > 0 ? nativeChain : chain;
    // 时长：上传视频节点上的 duration 是默认值，真实秒数才准 → 用真实秒数取最接近的档。
    const duration = closestWorkflowVideoDurationLabel(modelCandidates[0], sourceDurationSeconds) ?? sourceNode.data.duration;
    const data: WorkflowNodeData = { ...defaultNodeData("video"), prompt, model: modelCandidates[0], ratio, resolution, duration, videoReferenceMode: "reference", isRunning: true, startedAt: Date.now() };
    ...
    // 源视频精确时长传下去，让「参考总时长」校验拿到真实值（生成的 15 秒视频实际 15.1 秒）。
    void runVideoNode(node, {
      modelCandidates,
      referenceVideosOverride: [sourceUrl],
      referenceVideoDurations: sourceDurationSeconds !== undefined ? { [sourceUrl]: sourceDurationSeconds } : undefined,
      forceReferenceMode: "reference",
    });
    return node;
  ...
  const createVideoDepthNode = useCallback((sourceNode: WorkflowNode, options?: { position?: { x: number; y: number }; select?: boolean }) => {
    ...
    const sourceDims = sourceNode.data.videoDimensions ?? getWorkflowNodeNaturalSize(sourceNode);
    const sourceDurationSeconds = typeof sourceNode.data.durationSeconds === "number" ? sourceNode.data.durationSeconds : undefined;
    const outputDims = sourceDims?.width && sourceDims.height ? fitVideoDepthOutputSize(sourceDims.width, sourceDims.height) : undefined;
    const ratio = outputDims ? closestVideoDepthRatioLabel(outputDims.width, outputDims.height) : "16:9";
    // ⭐ 上游只处理前 MAX_DEPTH_SECONDS 秒、服务端也只按那么多秒收钱 → 界面上就得显示截断后的秒数，
    //   ⛔ 别显示源视频的完整时长（那会变成"卡片写 60 秒、成品只有 25 秒"）。
    const duration = sourceDurationSeconds && sourceDurationSeconds > 0 ? `${Math.min(MAX_DEPTH_SECONDS, Math.max(1, Math.floor(sourceDurationSeconds)))}秒` : sourceNode.data.duration;
    const resolution = DEPTH_OUTPUT_RESOLUTION;
    const data: WorkflowNodeData = { ...defaultNodeData("video"), prompt: "深度动作捕捉", model: VIDEO_DEPTH_MODEL_ID as ModelName, ratio, resolution, duration, enhanceSourceUrl: sourceUrl, isRunning: true, startedAt: Date.now(), videoDimensions: outputDims, durationSeconds: sourceDurationSeconds };
    const draftNode: WorkflowNode = { id: createId("workflow_node"), kind: "video", title: getNodeLabel("video"), x: 0, y: 0, data };
    const size = getWorkflowNodeVisualSize(draftNode);
    const position = options?.position ?? findNonOverlappingNodePosition(current.nodes, size, sourceNode);
    const node: WorkflowNode = { ...draftNode, x: position.x, y: position.y };
  ...
  return (
    <WorkflowRuntimeContext.Provider value={runtime}>
        ...
        <div className="lovart-workflow-dock pointer-events-auto flex items-center gap-1 rounded-[14px] border border-white/72 bg-white/92 p-1.5 shadow-[0_16px_34px_rgba(0,0,0,0.16)] backdrop-blur-[16px]">
          ...
          <input ref={uploadNodeInputRef} type="file" hidden multiple accept={`${IMAGE_UPLOAD_ACCEPT},${VIDEO_UPLOAD_ACCEPT},${AUDIO_UPLOAD_ACCEPT},.txt,text/plain`} onChange={(event) => { void handleUploadNodeFiles(Array.from(event.target.files ?? [])); event.target.value = ""; }} />
...
function WorkflowHoverTip({ label, shortcut, box }: { label: string; shortcut?: string; box: { left: number; top: number } | null }) {
  if (typeof document === "undefined") return null;
  return createPortal(
    <span className="pointer-events-none fixed flex -translate-x-1/2 -translate-y-full items-center whitespace-nowrap rounded-[8px] bg-[#1f2329] px-2.5 py-1.5 text-[12px] font-medium text-white shadow-[0_8px_20px_rgba(0,0,0,0.24)]" style={{ left: box?.left ?? 0, top: box?.top ?? 0, zIndex: 13000, visibility: box ? "visible" : "hidden" }}>
...
function WorkflowTipButton({ label, shortcut, onClick, className, children, wrapperClassName = "" }: { label: string; shortcut?: string; onClick: () => void; className: string; children: ReactNode; wrapperClassName?: string }) {
  ...
  useLayoutEffect(() => {
    if (!hovered || suppressed) return;
    const root = rootRef.current;
    if (!root) return;
    const rect = root.getBoundingClientRect();
    setBox({ left: rect.left + rect.width / 2, top: rect.top - 8 });
  }, [hovered, suppressed, label]);
  return (
    <div ref={rootRef} className={`relative flex ${wrapperClassName}`} onPointerEnter={() => setHovered(true)} onPointerLeave={() => { setHovered(false); setSuppressed(false); }}>
      <button type="button" onClick={() => { setSuppressed(true); setHovered(false); onClick(); }} aria-label={label} className={className}>{children}</button>
      {hovered && !suppressed ? <WorkflowHoverTip label={label} shortcut={shortcut} box={box ? { left: box.left, top: box.top } : null} /> : null}
    </div>
  );
}
...
function WorkflowAssetLayerRow({ asset, getImageDisplayUrl, getVideoPosterDisplayUrl, compact, onRestore, onDelete, onSelect, onFocus, selected, isLocked, isHidden, onToggleLock, onToggleHidden, dragType }: { asset: WorkflowAssetSummary; getImageDisplayUrl?: (url: string) => string; getVideoPosterDisplayUrl?: (url: string, posterUrl?: string) => string | undefined; compact?: boolean; onRestore?: () => void; onDelete?: () => void; onSelect?: () => void; onFocus?: () => void; selected?: boolean; isLocked?: boolean; isHidden?: boolean; onToggleLock?: () => void; onToggleHidden?: () => void; dragType?: string }) {
  ...
  const rawPreviewUrl = asset.kind === "video" ? getVideoPosterDisplayUrl?.(asset.url, asset.posterUrl) : getImageDisplayUrl?.(asset.url);
  const previewUrl = previewFailed ? undefined : rawPreviewUrl;
  return (
    <div draggable={Boolean(onRestore)} onClick={onSelect} onDoubleClick={onFocus} onDragStart={(event) => { if (!onRestore) return; const key = dragType ?? "application/x-flashmuse-workflow-asset"; event.dataTransfer.setData(key, asset.id); if (key === "application/x-flashmuse-workflow-asset") event.dataTransfer.setData("text/plain", asset.id); event.dataTransfer.effectAllowed = "copy"; }} className={`group relative flex items-center gap-2 rounded-lg px-2 text-[12px] text-[#6b7280] ${compact ? "h-9" : "h-10"} ${onRestore ? "cursor-grab hover:bg-[#f3f4f6] active:cursor-grabbing" : onSelect ? "cursor-pointer hover:bg-[#f3f4f6]" : ""} ${selected ? "bg-[#eef1f4]" : ""}`}>
      <span className="relative flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-md border border-[#dfe3e8] bg-[#f8fafc] text-[#5c626b]">
        {previewUrl ? <WorkflowHoverImagePreview src={asset.kind === "image" ? asset.url : previewUrl} alt={asset.name} wrapperClassName="block h-full w-full"><img src={previewUrl} alt="" className="h-full w-full object-cover" draggable={false} onError={() => setPreviewFailed(true)} /></WorkflowHoverImagePreview> : asset.kind === "video" ? <RiVideoLine className="h-4 w-4" /> : <RiImageAiLine className="h-4 w-4" />}
        {asset.kind === "video" && previewUrl ? <VideoPlayBadge size="xs" /> : null}
      </span>
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-[20px] border-[5px] border-[#b8b8b8] bg-white" style={{ height }}>
      <textarea
        ...
        style={{ font: "16px / 28px var(--font-sans)", pointerEvents: isEditing ? "all" : "none" }}
      />
    </div>
  );
}
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        style={{ maxHeight, font: "14px / 24px var(--font-sans)" }}
      />
    </div>
  );
}
...
function WorkflowPromptBox({ node, value, placeholder, maxPromptHeight, onChange, children, running, onRun }: { node: WorkflowNode; value: string; placeholder: string; maxPromptHeight?: number; onChange: (value: string) => void; children: ReactNode; running?: boolean; onRun: () => void }) {
  ...
  return (
    ...
    >
      {localTip ? <div className="pointer-events-none absolute bottom-full left-1/2 z-[13000] mb-3 -translate-x-1/2"><WorkflowInputToast message={localTip.message} exiting={localTip.exiting} /></div> : null}
            ...
            <div key={upload.id} className="group relative h-[70px] w-[64px] shrink-0 overflow-visible">
              <div className={`relative h-full w-full overflow-hidden rounded-[16px] border ${isVisual ? "border-[#e5e5e5] bg-[#f7f7f7]" : "border-[#e1e1e1] bg-white text-[#777777]"}`}>
                ...
                mediaSrc ? upload.kind === "video" ? (
                  <VideoUploadThumbnail src={mediaSrc} posterUrl={upload.url ? upload.previewUrl : undefined} alt={upload.name} />
                ) : (
                  // eslint-disable-next-line @next/next/no-img-element
...
function WorkflowModelMenuSingle({ value, options, title, onChange, className = "", getIcon, creditRate }: { value: ModelName; options: readonly (ConversationModel | GenerationModel)[]; title: string; onChange: (value: ModelName) => void; className?: string; getIcon?: (modelId: string) => typeof BytedanceIcon | typeof RiOpenaiFill | typeof RiGoogleFill | null; creditRate?: { usdToCnyRate: number; creditsPerCny: number } }) {
  const { open, setOpen, toggle } = useWorkflowMenuOpen();
...
function VideoNodeEditor({ node, modelOptions, promptMaxHeight, onChange, onRun }: { node: WorkflowNode; modelOptions: WorkflowModelOptions; promptMaxHeight?: number; onChange: (nodeId: string, patch: Partial<WorkflowNodeData>) => void; onRun: () => void }) { const model = modelOptions.videoModels.some((item) => item.id === node.data.model) ? node.data.model ?? DEFAULT_VIDEO_MODEL : (modelOptions.videoModels[0]?.id as ModelName | undefined) ?? DEFAULT_VIDEO_MODEL; const supportedResolutions = filterVideoResolutionsForMembership(modelOptions.membershipTier ?? "free", getSupportedVideoResolutions(model), modelOptions.membershipSettings); const resolution = normalizeVideoResolutionForModel(model, node.data.resolution); const supportedRatios = getSupportedVideoRatios(model, resolution); const ratio = (supportedRatios as readonly string[]).includes(node.data.ratio ?? "") ? node.data.ratio as string : supportedRatios[0]; const durationOptions = modelOptions.videoModels.find((item) => item.id === model)?.durations ?? fallbackVideoDurationOptions; const isVideoEditOrExtend = supportsVideoReferenceMode(model) && (node.data.videoReferenceMode === "edit" || node.data.videoReferenceMode === "extend"); const isPostProcessNode = isVideoDepthModel(node.data.model) || isVideoEnhanceModel(node.data.model); return <div className="space-y-2">
  {/* ⛔⛔ 画质增强 / 深度动作捕捉节点不许显示模型·参数·时长三个菜单：它们的模型压根不在
      `modelOptions.videoModels` 里 → 上面那行 `model` 会回落成列表第一个（Seedance 2.0 Fast），
      于是节点上显示的是**另一个模型**；更要命的是用户点一下模型菜单就把 `node.data.model` 改成了
      普通视频模型 → 这个节点从此拿「深度动作捕捉」当提示词真跑一条普通视频、真扣钱
      （和上一批修的"运行按钮掉进 runVideoNode"是同一族的另一半）。
      ⭐ 这些节点的档位由快捷菜单在创建时定死（深度恒 480p，增强由用户在下拉里选），本来就不该在这里改。 */}
<WorkflowPromptBox node={node} value={node.data.prompt ?? ""} placeholder="输入提示词，也可以连接文本节点" maxPromptHeight={promptMaxHeight} onChange={(value) => onChange(node.id, { prompt: value })} running={node.data.isRunning} onRun={onRun}>{isPostProcessNode ? null : <WorkflowModelMenuSingle value={model} options={modelOptions.videoModels} title="选择模型" onChange={(value) => { const nextResolution = normalizeVideoResolutionForModel(value, node.data.resolution); onChange(node.id, { model: value, resolution: nextResolution, ratio: normalizeVideoRatioForModel(value, ratio, nextResolution), duration: value === DEFAULT_WORKFLOW_VIDEO_MODEL ? "8秒" : modelOptions.videoModels.find((item) => item.id === value)?.durations?.[0] ?? "5秒" }); }} className="w-[190px] shrink-0" creditRate={modelOptions.creditRate} />}{isVideoEditOrExtend || isPostProcessNode ? null : <WorkflowSettingsMenuSingle mode="video" model={model} ratio={ratio} resolution={resolution} ratios={supportedRatios} resolutions={supportedResolutions} onChange={(patch) => onChange(node.id, patch)} className="shrink-0" />}{isVideoEditOrExtend || isPostProcessNode ? null : <WorkflowDurationMenuSingle value={node.data.duration ?? durationOptions[0]} options={durationOptions} onChange={(value) => onChange(node.id, { duration: value })} />}</WorkflowPromptBox></div>; }

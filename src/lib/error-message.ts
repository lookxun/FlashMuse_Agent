...
export function toUserErrorMessage(value: unknown, fallback = "请求失败，请稍后再试。") {
  ...
  if (/(input image|reference|asset|素材|参考图|审核).*(copyright|copyright restrictions|related to copyright|版权)|reference-review-failed-copyright/.test(lower) || /审核图片可能涉及版权限制|参考图.*版权|素材.*版权/.test(text)) return withErrorCode("审核图片可能涉及版权限制，平台已拒绝，请换其它图片重试。");
  if (/copyright|copyright restrictions|related to copyright|版权/.test(lower)) return withErrorCode("输出视频可能涉及版权限制，平台拒绝生成。重新生成有可能会成功。");

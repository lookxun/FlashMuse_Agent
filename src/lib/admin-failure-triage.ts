 ...
 */
export const FAILURE_REASON_SQL = `regexp_replace(regexp_replace(regexp_replace(regexp_replace("failureReason", '^\\(B_[0-9]+\\)\\s*', ''), '^(图片平台没有返回图片)：.*$', '\\1（模型未产出或拒绝生成）'), '^(模型因色情/暴力/隐私安全等原因拒绝出图).*$', '\\1（附模型拒绝原文）'), '^(模型这次没有出图，只回了一段文字).*$', '\\1（附模型返回原文）')`;

...
8. ⭐⭐ **按规矩跑了最便宜的冒烟**：新建对话「v1.0.1.29 正式服冒烟：一切正常。」→ 把语音模型切成
   **Qwen Audio 3.0 TTS Plus** → 出 4 秒音频（音色 龙安灵心）；
   `CreditLedger` = `09-13 00:09:18 | audio | qwen/qwen-audio-3.0-tts-plus | credits 1 | usd 0.00042`；console 0 error。

### 六之二、⭐⭐⭐ 用户澄清：**下架的只是 Fish「免费版」，收费版好着呢，而且它就是菜单默认项**

🗣️ 用户原话：「**本来有两个 fish 一个是免费的一个是收费的。openrouter 下架的是免费的那个。
我们平台后台我关掉了。所以你现在首选的那个 fish 是收费的。他是没问题的。你可以用这个收费的 fish 跑冒烟。**」

| 模型 id | 菜单名 | `usdPerChar` | 状态 |
|---|---|---|---|
| `fish-audio/s2.1-pro-free` | Fish Audio S2.1 Pro（免费） | 0 | ⛔ 被 OpenRouter 下架，**且已被用户在后台关掉 → 菜单里压根不出现** |
| `fish-audio/s2.1-pro` | Fish Audio S2.1 Pro | 0.000015 | ✅ **正常可用**，一句话 1 积分，**现在菜单里默认选中的就是它** |

- ⛔⛔ **第 127/128 次会话写的「OpenRouter 把 fish-audio 全系下架」「新对话默认语音必红字」是错的，别再往下传。**
  本批的旁证：正式服语音菜单里只有 **3 行**（收费 Fish / Qwen / MiniMax），**没有「免费」那一行**
  → 说明后台开关确实把免费版过滤掉了，**用户正常操作已经撞不到那个坏模型**。
- ⭐ **所以冒烟可以直接用菜单默认的收费版 Fish，不用手动切模型**（本批我仍手动切到了 Qwen，两个都是 1 积分、都行）。
- ⚠️ **代码层留个心眼**：`DEFAULT_AUDIO_MODEL = audioGenerationModels[0].id` 仍指向**免费版**那个 id
  （`models.ts:536/542`），菜单之所以落在收费版，**全靠后台开关把免费版过滤掉**
  → ⛔ 谁在后台把免费版重新打开，红字 `B_498/B_500` 就会回来。
- ⭐⭐ **方法论教训（已写进 `03` 和 `05`）**：**`GET /api/v1/models` 列表里查不到 ≠ 该模型不可用。**
  上一批只拿免费版的 id 直打验证过（确实 404），就把收费版**一起判了死刑并写进交接文档**
  → **要断言某个模型挂了，必须拿那个确切的 model id 直打一次。**
- 📌 **已同步修正的文档**：`AGENTS.md`（部署铁律的冒烟候选）、`03-deploy-and-servers.md`（候选表 + 历史背景节）、
  `00-README.md`、`01-current-status.md`、`05-next-actions.md`（那条"要不要改默认语音"的待办**已撤销**）。
  ⛔ 历史会话记录（第 127/128 次那几节）**原样保留不改**（那是当时的真实认知，改了等于伪造记录），
  只在本节和上面那几处加了"已被推翻"的指向。
...
5. ⭐⭐ **"供应商列表里查不到" ≠ "这个模型不可用"** —— 要断言某模型挂了，必须拿那个**确切的 model id 直打一次**
   （上一批只验了 Fish 免费版就把收费版一起判死，写进文档误导了两批）。
6. ⭐ **冒烟可用清单更新**：Qwen Audio 3.0 TTS Plus **或** Fish Audio S2.1 Pro（**收费版，菜单默认项**），
   一句话都是 1 积分；⛔ 只有免费版 `fish-audio/s2.1-pro-free` 不能用（已下架 + 已被后台关掉）。

...
- **[x] M003** 正式服工作流已开（compose/Dockerfile `NEXT_PUBLIC_WORKFLOW_MODE_ENABLED=true`）。**[x] M005** @mention 已收敛到 `mention-text.ts`。**[x] M009** BytePlus 审核后持久化 `bytePlusAssetId`、视频以 `asset://` 发送已落地。**[x] M012** 对话流 TTS + Fish 音色克隆已落地。
- **[x] M041 / M038 / M039** —— ✅ 第九十一次写完，第九十三次已随 `v1.0.1.10` 上正式服。
...
- **[x] M045** 语音默认模型「是不是坏的」—— **2026-09-12 用户澄清后直接关闭**（下架的只是 Fish **免费版**且已被后台关掉，
  菜单默认落在**可用的收费版** `fish-audio/s2.1-pro`，没有"默认就红字"这回事）。详情见下方 M045 节。
...
### [x] M042 ⭐⭐⭐ 接真支付（积分充值）—— 2026-09-05 加；**2026-09-09 审核已通，测试服 v1.0.1.19 已出码付钱**

- **已完成**：下单、回调验签、幂等加分、真码 UI、微信先藏、PaymentOrder、后台 8 档可调、三种付款状态。支付宝审核已过，本地/测试服都能付成功。
- **还没完成**：正式服还没这套代码，也还没配 `ALIPAY_*`。等用户说才推。
- ⚠️ 会员那套（`applyMembershipPurchase`）**先别接**。

### [x] M044 深度动作捕捉「尺寸太大必挂」—— 2026-09-12 加；**同日用户改口径关掉**

- **原问题**：1280×704 × 123 帧在 RunningHub 必挂 CUDA；896×512 立刻成功。当时口径是「成品必须跟源视频一样」，所以只能等拍板拦不拦。
- **2026-09-12 第一百二十九次用户拍板**：不要原尺寸了。统一 480p、只出 16:9 / 4:3 / 1:1（竖版对应）、21:9 贴 16:9、最长 25 秒。
  唯一权威 `src/lib/video-depth-size.ts`。真机 16:9 25 秒连成两次、30 秒 OOM。
- ⛔ 别再提「要不要前置拦截大尺寸」——口径已经换成降档输出。
- ⭐⭐ **2026-09-12 第一百三十次补一条（同族但更隐蔽）**：光封顶秒数**治不了 60fps 的素材** ——
  `帧 = min(25秒, 时长) × 源fps`，fps 原来放到 120 → 60fps 跑满 25 秒 = 1500 帧 = 实测天花板（750）的 2 倍、必挂。
  已把 fps 封顶 **30**（`DEPTH_MAX_FPS` / `resolveVideoDepthFps`）。⛔ 别用"截帧数"代替"降 fps"（那会多收钱）。

### [x] M045 ⭐ 语音默认模型「是不是坏的」—— 2026-09-12 第一百三十次由用户澄清后**直接关闭**

- **曾经的误判（第 127/128 次会话写进文档的）**：「OpenRouter 把 fish-audio 全系下架 → 新对话默认语音模型必红字，
  要不要把 `DEFAULT_AUDIO_MODEL` 改成 Qwen / 把 fish 藏掉」。
- 🗣️ **用户澄清（2026-09-12）**：「本来有两个 fish 一个是免费的一个是收费的。openrouter 下架的是免费的那个。
  我们平台后台我关掉了。所以你现在首选的那个 fish 是收费的。他是没问题的。你可以用这个收费的 fish 跑冒烟。」
- ✅ **结论**：`fish-audio/s2.1-pro-free`（免费版）= 已下架 **且已被后台关掉、菜单里不出现**；
  `fish-audio/s2.1-pro`（收费版）= **可用，一句话 1 积分，且就是菜单默认项** → **没有"默认就红字"这个问题，不用改代码。**
- ⚠️ **唯一要留意的**：`DEFAULT_AUDIO_MODEL = audioGenerationModels[0].id` 在代码里仍指向**免费版 id**，
  菜单能落在收费版**全靠后台开关过滤** → ⛔ 谁在后台把免费版重新打开，红字 `B_498/B_500` 就会回来。
  真要"彻底防呆"才需要改代码（把默认值指到收费版或 Qwen），但**用户没要求，先不动**。

### [x] M043 ⭐ 预估价目表定期回校 —— 2026-09-05 加；**2026-09-12 已按 GPT Image 2.5 回校过一次（仍开着）**

- **是什么**：`models.ts` 的 `estUsdByResolution`（图片）/ `estUsdPerSecondByResolution`（视频）
  是**用正式服真实扣费数据（`CreditLedger`）的 p99 统计出来的**，只给额度闸门的事前预估用。
- **什么时候要回校**：① 供应商调价 ② 接新模型（新模型没条目 = 预估回落到基准价，可能很不准）
  ③ 用户又反馈"该拦的没拦 / 不该拦的被拦了"。
- **怎么回校**：SQL 在 `05-next-actions.md` 的「预估表怎么重新统计」那一节，⭐ 取 **p99** 填表。
- ⚠️ 改完必须跑回归确认**菜单副标题一个字没变**（同一张表被"界面报价"和"闸门"两个读者共用）。
- ⭐⭐ **2026-09-12 回校 GPT Image 2.5 时学到的（下次照做）**：
  ① 决定价钱的维度**不一定是分辨率** —— 2.5 主要看**画质**（max 是默认档 4 倍）、其次看**比例**
     （越"方"越贵，1:1 是 16:9 的 1.85 倍）→ 为此新增了 `estQualityMultiplier`（**只配给 2.5 两个模型**）；
  ② 基准取「该分辨率里**最贵比例**的默认画质」，闸门宁高不低；
  ③ **`usd` 字段同时被 `getImageModelFallbackUsd`（上游不给成本时的扣费兜底）读**，改它要意识到；
  ④ ⭐ **改完先跑 `npx tsx scripts/verify-generation-pricing.ts`（57 条，含"别的模型一个字没变"的反向用例）**。
- ⚠️ **2.5 的 2K / 4K 档还没有真实样本**（现在 0.11 / 0.16 是按 GPT-5.4 的 2K/1K≈2.08 倍推的）→ 正式服跑出真实扣费后回校。

### [x] M041 ⭐ 简繁转换会「改用户自己的字」+ 繁体下会显示过期文字 —— ✅ **2026-08-26 第九十一次会话完成**

**来源**：修「测试服公告『新增』显示成『新建』」时挖出来的。那个 bug 本身**已修好**
（根因 = 简体模式下跑了一张**机械反转**出来的有损"繁→简"词表，里面有 `["新增" → "新建"]`；
现已改成「简体只还原存下来的原文、绝不做替换」并删掉那个反向函数，见 `chat-workbench-core.tsx:1509` 附近的注释）。
⭐ **下面两条是同一套机制的剩余问题，只影响繁体用户，简体用户已完全安全。**

🗣️ **用户的原则(要当口径记住)**：「**用户用什么文字打,显示出来就是什么文字**」——
现在这套机制对繁体用户**违反了**这条原则。

**问题 A（较严重）：简繁转换分不清「界面标签」和「用户自己的内容」。**
`applyDocumentLanguage` 是**把整个 `document.body` 的文字统统查找替换一遍**，
排除名单只有 `script, style, noscript, textarea, input, [contenteditable="true"], [data-no-translate="true"]`
（`chat-workbench-core.tsx:1518`）。而 `data-no-translate` 全项目**只用在少数 toast 提示上**
（`grep data-no-translate` 只有 3 处）→ **用户内容一个都没被保护**，已确认两处：
- **发出去的聊天消息**（打字时在 contenteditable 里是安全的，一发出去变成普通文本节点就会被转）；
- **上传文档的预览正文**（`chat-workbench-core.tsx:6809` 那个 `<pre>`，整篇被查找替换）。
- 同理还有：**顶部公告**（管理员打的内容）、提示词、素材名等。
→ 繁体用户打「新建视频」发出去会**变成「新增影片」**。

**问题 B（较轻）：繁体下内容更新后会显示过期文字。**
繁体分支（`chat-workbench-core.tsx:1520~1524`）在节点**已存过原文**时，
拿的是**小本子里那份旧原文**重新转换写回，而**不是刚变出来的新内容** →
`characterData` 变化（积分数字刷新、AI 回复流式逐字冒出）会被**旧内容覆盖**，看起来像"卡住/不更新"。

**以后怎么做（两个可独立进行）**：
- 修 A = 给**用户内容**的渲染处标 `data-no-translate="true"`（公告横幅、聊天消息、提示词、文档预览、素材名）。
  ⭐ 判据一句话：**这段文字是"我们写的界面文案"还是"用户/管理员打进去的内容"**，后者一律标上。
  ⚠️ 注意 `applyLanguageToTextNode` 用的是 `parent.closest(...)`，所以标在**外层容器**上即可覆盖整棵子树。
- 修 B = 繁体分支改成「以**当前** `nodeValue` 为准重新存原文」而不是死用旧原文；
  ⚠️ 但要想清楚"怎么区分『内容真的变了』和『我们自己刚写进去的繁体文字』"（否则会自己转自己、无限循环）——
  这是这条不那么好做的原因，**别低估**。

**为什么押后**：繁体用户目前应该极少（默认简体），而本次要先把"影响所有用户"的公告改字 bug 收工上线。
⛔ 别把这两条和公告那次修复混在一起做。

### [x] M040 ⭐ 把「红字映射幂等」固化成 `npm test` 用例 —— ✅ **2026-08-09 第六十一次会话完成（v1.0.0.95）**

**成品 = `tests/error-message-idempotency.test.ts`（56 个用例，`npm test` 从 15 → 71 全过）。**

⭐ **设计上的关键决定（以后改这个测试先看这条）**：**故意不 import 任何内部文案常量**
（`buildModelRefusedMessage` / `PROVIDER_INSUFFICIENT_CREDITS_MESSAGE` 这些本来就没导出，
也**不要为了测试去导出**）。测的是**不变量** `f(x) === f(f(x)) === f(f(f(x)))`，不是"某句话长什么样" →
以后改措辞不用改测试，而措辞改坏了幂等仍然会被抓住。

覆盖：
- **45 条真实上游原文**各自连跑 3 遍（原文全部来自 `error-message.ts` 注释与线上诊断日志，⛔ 不是臆造的英文）；
- **`(B_xxx)` 前缀**：不丢 + 仍幂等；
- **9 条反向用例**：英文必须被映射（不许原样透出）／**B_123 回归**（参考视频没过审 ⛔ 不许出现"拒绝出图/拒绝原因"）／
  图片·视频·**音频**三种类型各自对应（audio 历史上漏写过）／**句中假冒**不许被当成成品放过／
  **近似句**（「参考视频**通过了**版权检测」）不许被误判／成品被拒不许错怪参考素材／
  限流不许说成余额不足／"我们没配密钥"不许说成"密钥已过期"；
- ⭐ **顺带固化了一个我原以为存在、实测不存在的边界**：末尾「中文透传 + 超长截断」那一路
  **是幂等的**（`slice(0,180)+"..."` = 183 字，第二遍再截 180 正好把 `...` 削掉又加回来）。
  已加 176~200 逐长度扫描 → **改 `maxLength` 或省略号写法的人会被这条挡住**。

**本次实跑结果：56/56 全过，一条 BREAK 都没有** → 说明现有那道白名单幂等保护是对的，⛔ 没有改任何行为。

### [x] M040-历史背景（保留，改红字文案前必读）
...
### [x] M038 `mention-text.ts` 的「删 @名」和「解析 @名」两个正则不对称 —— ✅ **2026-08-26 第九十一次会话完成**

**修法**：`removeMentionName` / `replaceMentionName` 的 lookahead 加上 `@`，与 `getMentionNames` 对称。`@000@A_old` 能删其中一个；`@000_2` / `@000abc` 不误伤。

<details>
<summary>原备忘（保留备查）</summary>
...
### [x] M039 `getOrderedExplicitImageReferences` 会从整个资产库把 @名对应的图捞回来并排最前面 —— ✅ **2026-08-26 第九十一次会话完成**

**产品口径（用户拍板）**：有图没 @ 正常；点文件名/@按钮才会同时有图和 @；单独 @ 无效果、当普通字。**禁止按 @名 从整个资产库捞图。** 使用提示词从那条消息/生成任务带当时的参考。
...
### [x] M037 把上传进度 `uploadProgress` 从画布状态里搬出去 —— ✅ **2026-08-10 第六十三次会话完成（v1.0.0.97）**

**实际做法（比"独立 Record state"更省、更低风险）**：没有把 `uploadProgress` 从 `node.data` 里彻底搬走，
而是新增 `updateNodeUploadProgress(nodeId, progress)`（`workflow-tldraw-canvas-inner.tsx`）——
进度只直接 `editor.updateShape` patch 那**一个** shape 的 `props.node.data.uploadProgress`，tldraw 自己只重渲染那一个节点的
`UploadingNodeOverlay`。**不 `exportStateFromEditor`、不 `stateKey`（整张画布 `JSON.stringify`）、不 `onChange`、不 PUT。**
配套 `progressOnlyUpdateRef`：`registerAfterChangeHandler` 的 workflow_node 分支在进度更新期间直接 `return`。
4 个上传热点回调（图/视频/音频/文本）换成它；**上传完成/失败仍走原 `updateNode`**（带真实 url + `uploadProgress:undefined`）正常落库。
既有 `throttleUploadProgress` 节流保留、存库边界剥离逻辑不动。

⚠️ **遗留（未做，也不急）**：`uploadProgress`/`uploadPreviewUrl` 仍在 `node.data` 上，靠存库边界的剥离函数兜着
（备忘原本设想的"从根上让它们不在画布状态里"没做）—— 现有剥离一直有效，本次不动它。真要根治再按备忘原方案挪进独立 map。
...
### [~] M029 对话流生成「统一单轮询器」—— ⚠️ **重构本身仍未做；但 2026-08-10（第六十三次会话）已修掉它导致的"视频双失败卡"具体症状**

- ✅ **2026-08-10 修掉了具体症状（v1.0.0.97）**：用户报"视频双失败卡"（一条视频消息里两张失败卡、B_242）。
  ⭐ 关键认知：**不是"两个轮询器都 +1"这么笼统** —— 图片失败按 slot 下标幂等（标两次仍 1 张），**只有视频是无脑
  `failedVideoCount+1`**。前台 `while` 轮询 + 后台 reconcile 跨重启/竞态下对同一个视频双双收尾 → 计数变 2。
  修法：`markAssistantVideoFailure` 加"只有还剩待生成名额(`pendingVideoCount>0`)才计一次失败"守卫（保住不变量
  `videos+failed+pending=请求数`）。历史坏数据本地 1 + 正式 2 已手动收回。
- ⚠️ **重构本身（下面这些）仍未做**，只是那个最痛的症状不再发生了：
...
- **问题**：对话流的生成状态有**两个轮询器**——前端的 `while` 循环 + 数据驱动的后台 reconcile。
  两者都会 `+1` 失败计数器 → 历史上**出过多次"重复失败卡"**（视频这条 2026-08-10 已止血，见上）。
...
### [x] M025-旧 上一任评估留档 —— 非待办；M025 本体已上线，下文只作方法论参考
...
### [x] M005 输入框 @mention 重构 —— ✅ 已收敛到 `mention-text.ts`（含 contenteditable 选区原子化）
- 2026-08-23 对代码核销：对话流/工作流共用唯一权威，不再各写一份。
- 剩余独立缺陷见 **M038**（删/解析正则不对称）、**M039**（@ 从资产库捞回旧图），别把这条重开。
...
### [x] M009 BytePlus 审核 asset-url 流程完善 —— ✅ 已落地
- 2026-08-23 对代码核销：`/api/video` 送审后把 `bytePlusAssetId` 写入 `UserAssetState`，过审参考以 `asset://` 发给上游；`generation-jobs.resolveReferenceUrls` 再解析回可显示 url。
- 「本地 `/generated` 改公网 HTTPS 直链发给上游」仍是 **M001**，别和这条混。
...
### [x] M011 清理重复 `.env.local` —— ✅ **2026-08-09 只读勘察确认：问题已不存在，可关**
- 原始押后理由：正式服现在能用，改 env 有风险。以后在服务器上小心清理重复 `DATABASE_URL` 行（第一个是对的，psql 要去 `?schema=`），别暴露密钥。
- ⭐ **2026-08-09 第六十一次会话实测（只数 key 名和次数，⛔ 一个值都没打印）**：
  - `/opt/flashmuse/data/.env.local`（正式）：48 行 / 39 个有效赋值，**重复 key 数量 = 0**，`DATABASE_URL` 恰好 1 行；
  - `/opt/flashmuse-staging/data/.env.local`（测试）：53 行 / 42 个有效赋值，**重复 key 数量 = 0**，`DATABASE_URL` 恰好 1 行。
- **结论：不用做了。** 那些重复行应该是在过去某次重建 env 时已顺手消掉的（历史无记录）。
- ⭐ 判据留档（以后想复查照抄，成本几秒、零风险）：
  `sudo grep -oE '^[[:space:]]*[A-Za-z_][A-Za-z0-9_]*=' <env> | tr -d ' ' | sed 's/=$//' | sort | uniq -c | awk '$1>1'`
  —— **只输出 key 名和次数，绝不打印值**（env 里全是密钥/数据库口令）。

### [x] M012 声音克隆 / TTS —— ✅ 对话流已落地（第七十～八十次会话）
- 2026-08-23 对代码核销：`/api/audio` + MiniMax / Qwen / Fish TTS；Fish 另有「文本转换 / 音色克隆」。不是靠视频参考音做人声克隆。
...
### [x] M003 正式服工作流模式 —— ✅ 早已开放，无待办
- 仓库 `docker-compose.yml` / `Dockerfile` / staging compose 均为 `NEXT_PUBLIC_WORKFLOW_MODE_ENABLED=true`。

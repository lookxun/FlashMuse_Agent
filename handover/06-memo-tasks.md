...
- **[x] M003** 正式服工作流已开（compose/Dockerfile `NEXT_PUBLIC_WORKFLOW_MODE_ENABLED=true`）。**[x] M005** @mention 已收敛到 `mention-text.ts`（剩余缺陷另记 M038/M039）。**[x] M009** BytePlus 审核后持久化 `bytePlusAssetId`、视频以 `asset://` 发送已落地。**[x] M012** 对话流 TTS + Fish 音色克隆已落地。
...
### [ ] M041 ⭐ 简繁转换会「改用户自己的字」+ 繁体下会显示过期文字（2026-08-09 第六十一次会话立项，**用户拍板先记备忘、这次不做**）

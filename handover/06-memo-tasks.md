...
### [ ] M044 🗳️ 深度动作捕捉「尺寸太大必挂」要不要加前置拦截 —— 2026-09-12 加（**等用户拍板**）

- **实测坐实（不是猜）**：同一条素材
  **1280×704 × 123 帧 → RunningHub 那台 GPU 必挂**（`torch.AcceleratorError` /
  `CUDA error: invalid configuration argument`，连挂两次）；
  **896×512 × 123 帧 → 立刻成功**（`usd 0.12` / 8 积分）。
  → 是**上游算力上限**，不是我们参数写错（尺寸已确认是源视频实测值、不是客户端声明值）。
- **本批只做了"让用户看得懂"**：新增错误映射 → **B_7「深度动作捕捉的算力节点执行失败了，请换一条更短或分辨率更低的视频后重试。」**
  （原来落在「服务器繁忙」兜底桶里）。⛔ 行为一个字没改。
- **要彻底不让用户撞见，只有两条路，都要用户先定口径**：
  ① **前置拦截**：超过某个「像素 × 帧数」就在点按钮那一刻直接提示，不去建任务
     （好处：不浪费 RunningHub 的 GPU 计费；坏处：大视频直接不能用）；
  ② **自动降尺寸 / 降 `window_size`**：能跑通，但**违反已拍板的口径「成品尺寸必须跟源视频一样」**
     （2026-09-11 用户明确说过 ⛔ 不许用 ffmpeg 硬拉尺寸，因为会变形）。
- ⚠️ **还缺一个数**：现在只知道 896×512 行、1280×704 不行，**中间那条线在哪没测**
  （要定阈值就得再花几次 GPU 钱去二分）。
- ⚠️ 顺带留档：**失败的 RunningHub 任务大概也会计他们的 GPU 时间** → 这类"注定失败"的请求是真花钱的。

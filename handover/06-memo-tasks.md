...
### [ ] M042 ⭐⭐⭐ 接真支付（积分充值）—— 2026-09-05 加；**2026-09-08 代码已上测试服，卡在支付宝应用审核**

- **已完成**：下单接口、回调验签、幂等加分、真码 UI、微信先藏、PaymentOrder 表、测试服 v1.0.1.16 已部署。
- **还没完成**：支付宝开放平台应用 **未上线**（实测 `not-online-app`）。审核过了再测出码。
- ⚠️ 会员那套（`applyMembershipPurchase`）**先别接**。
- ⚠️ 正式服还没这套代码，也还没配 `ALIPAY_*`。

### [ ] M043 ⭐ 预估价目表定期回校 —— 2026-09-05 加

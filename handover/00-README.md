...
> **⭐⭐ 最新（2026-07-21 later 测试服迭代 session）：已到【测试服 v1.0.0.34】，⚠️ 正式服仍 v1.0.0.25 / `c19ecca`、GitHub 未动、本地未 commit，`tsc` 过，有 1 个新 Prisma 迁移 `20260721000000_media_asset_duration_float`。用户明确交代【下一个 AI 直接部署正式服】。** 本对话在承接的 07-21 本地批（道具图片 prop_image + 工作流用量计数修复）之上，做完并部署测试服（v25→v34）：① **道具生成风格修正**（写实/2D/3D 作用在道具实物上，写实=手办产品照不出真人）+ **propify 扩展**（照片/海报/画作/书刊等印刷品·影像制品本身就是道具、表面可印人物，只有无载体活体主体才转手办）；② **B_232 参考视频总时长精度全平台修**（`durationSeconds` Int→Float 迁移 + 存精确 0.1s + 唯一校验 `validateReferenceTotalDuration` >15.0 拦、文案带 XX.X 秒 + video/route 权威 + 对话流/工作流发送前）；③ **B_252 音频(.bin)误入图片槽修**（服务端按真实 mediaType 剔除混进图片槽的音视频，两流共用 + 对话流三处过滤）；④ **资产库生成刷新/重启恢复统一**（恢复保留 generating + resume 续 poll；`assetsOnly` 响应返回 `assetGenerateJobs` + 客户端合并）；⑤ **预览页参考缩略图真·从数据库读**（新增 `/api/generation-references`）+ **道具生成@名与参考图脱钩根治**（实时剪草稿 + 提交按@名为唯一真源）+ **回填历史坏数据**（测试服已回填 asset_12/14_prop，**正式服待回填**）。**下一个 AI：① 整份 rsync 测试服→正式服(带迁移，v1.0.0.34 不自增) ② 正式服 DB 跑 `backfill-prompt-mentions.js` ③ commit+push(含更早道具批) ④ 抽验。** 详见 CHANGELOG / 01-current-status / 05-next-actions 顶条。

> **⭐⭐ 最新（2026-07-21 本地 session，⚠️ 全部仅本地/未部署/未 commit，`tsc` 过，无 Prisma 迁移）：** 承接线上 v1.0.0.25（`c19ecca` 未动）。本对话做了两件事：① **工作流右上角用量面板"视频计数虚高"修复**（真因=`addWorkflowGeneratedAssets` 累计计数用 stale `assets` 闭包去重、并发双收尾重复+1；修=`WorkflowCanvasState` 加持久 `countedGeneratedUrls`、去重挪进函数式更新、旧数据按真实节点自愈）；② **资产库第1组新增"道具图片"`prop_image`**（角色/场景/**道具**/分镜同组同款，图标 `RiBellLine`，三档比例 单道具9:16/多角度16:9/**四宫格1:1**(新增 `grid-square`)，约10个前后端文件，无迁移，`currentCategory`/`creditSource` 均 text 列）+ **道具化转换 propify**（人/角色/生物→手办/雕像摆件；场景→微缩模型/沙盘；分镜/剧情→代表性实体物品；只产实体道具绝不产真人真场景）。本地 DB 已把 10 张误分类道具图修回 `prop_image`，并**已替用户删本地 `.next`** 供干净重启。**踩坑铁律：本地 dev 改 API route/lib 后必须删 `.next` 干净重启，否则单路由跑旧编译会静默把 `prop_image` 兜底成 `conversation_images`（名字带 prop 对、分类落 conversation）。下一个 AI：待用户干净重启+硬刷验收 → 走测试服→正式服部署铁律 + commit+push。** 详见 CHANGELOG / 01-current-status / 05-next-actions 顶条。

> **⭐⭐ （上一线上态）2026-07-20 收尾：四方同步 正式服=测试服=本地=GitHub=`v1.0.0.25` / commit `c19ecca`，工作树干净、无 Prisma 迁移、四域名 200。** 本对话做完并全部上线正式服：① **工作流"断线后输入框永久转圈'加载引用资产...'"根治**——真因=断线/删缩略图漏删对应 @名(命名去重 vs 原始不一致)产生孤儿@名 + 孤儿@名触发 `loadMentionAssetFilters` 无限重载(missingFilters 把空分类永远算缺失)；修=新增自愈 effect(validReferenceNames 收缩就删失效@名，覆盖断线/删节点/删缩略图/切模式) + @名有效性改为"只认当前缩略图"(`validReferenceNames`=visibleUploads，粘贴裸@名不变蓝不加载) + 去掉读整库机制/转圈 + 取名统一 + missingFilters 兜底。**全平台统一规则：有缩略图才有效变蓝，没缩略图@名一定一起没。** ② **B_42 修复**：工作流融合生视频 @引用视频/音频被当参考图发 BytePlus(`content[N].image_url not an image`)，真因=`getWorkflowPromptReferenceUrls` 目录解析只认 image 不看 asset.kind；修=按 `(asset.kind ?? "image")===kind` 路由(图/视频/音频各归各槽)。以前没暴露因"@引用视频/音频"是 07-21 才加、之前只能+上传走正确分支、且该验证一直没跑。 ③ 对话流"使用提示词"媒体由累加改**整体替换**。 已用测试服号 `12424740@qq.com` 浏览器实测全部通过。**测试服账号明文见 03-deploy-and-servers.md（主测试号 12424740@qq.com / dragonstar；模拟真实测试优先用它、白名单号不测、测试内容别删）。下一个 AI 无遗留待推/待部署。**
>

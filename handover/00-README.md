...
- ⭐ **只剩一个待你拍板的项**：给 `DATABASE_URL` 加 `connection_limit`（25~30）（A4 真修复，属改环境变量+重部署，目前零复发 → 不急）。
  ⛔ 我原来提的"参考图缩尺寸"**已被用户否掉，别再提**。
...
7. `07-red-error-triage-and-archive.md` — ⭐ **红字失败原因排查方法论 + 归档规则（排查线上报错必读）**
   · **第十六节 = ⭐⭐ A 表剩下 6 条全部收口（A1/A2/A4 加映射、A6/A8 不用动、A9 改不了）+ 2 个待拍板的行为变更**
   · **第十五节 = ⭐ A7 查清（curl/ffmpeg 早就好了）+ 修掉"错误映射说假话"**
   · **第十四节 = ⭐⭐ A5 修完（base64 参考图）+ A3 查清（"零日志"是错的）+ 两个顺带真问题**
   · **第十三节 = ⭐⭐ 红字全量归纳成 A/B 两张表（A 表已全部标注处理结果）+ 归档脚本的全局护栏**

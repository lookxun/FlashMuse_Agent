...
### 🎯 待办 2：两个"已报备、用户一直没表态"的点（从上一会话继承）

1. **「从当前画布选择」是按节点连线，不是按单张图** → 源节点里有 4 张图时会一起连进来
   （连线语义本来如此，超限会被拦）。要改成"只连那一张"需要新机制。
2. **侧边栏三态没做持久化**（刷新回常规态）。我问过「要不要存 localStorage」，用户没回答。

### 🎯 待办 3：红字（⛔ 别自己开工）

正式服待排查 **38 条 / 5 种**，其中**审核类 33 条**（模型/平台拒绝，按铁律**改不了、不归档、就该一直亮**），
**兜底桶仅 2 条**（5.3%，很干净）。🗣️ 用户交代**等红字攒多了再排查**，⛔ **别跑归档脚本**。

### 本会话改动过的文件（4 个，都不是应用代码）

```
deploy/staging/flashmuse-test-8080.conf            加 upstream fm_test_app + keepalive + map；8080 入口
...
⭐ **应用代码一行没改**（v1.0.0.60 的代码就是上一会话那份，本次只是搬到正式服 + 改 nginx）。

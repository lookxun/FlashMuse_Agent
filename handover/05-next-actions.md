...
9. ⭐ **语音**：fish-audio 被 OpenRouter 下架，用户拍板先不管。
    ⛔ 连带使「正式服用免费语音冒烟」这条部署判据失效（`03` 里已有零成本替代四项）。
...
1. ⭐⭐ **判「新 key 有没有配上」看「容器内 `/app/.env.local` 有没有那几行」+「后台显示已启用」**，
    ⛔ **别看 `docker exec ... echo $XXX_API_KEY`** —— compose 没注进进程环境，
    app 是运行时读文件的，那个变量**本来就是空的**（本批差点误判）。
2. ⭐ **给正式服补 key，值一律从「已验过的测试服 env」原样抽**，⛔ 别从本地 `.env.local` 复制
    （本地那份历史上被写坏过一次）。
3. ⚠️ **`docker compose up` 的进度走 stderr，PowerShell 会渲染成红色 `NativeCommandError` —— 那不是失败**。

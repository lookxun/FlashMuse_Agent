...
## ✅ 当前状态（2026-08-09 第五十五次会话末）：**四方全部 `v1.0.0.87`，本批已 commit + push**

| | 版本 / 状态 |
|---|---|
| 本地 / 测试服 / 正式服 / GitHub | 全部 `v1.0.0.87`（commit `a1a6e81`）|

⭐ 本次三段（整批上两服 / 修失败趋势保留期 / 新增公告禁测铁律）细节见 `01-current-status.md` 顶条。

---

## 🎯 待办 1：Seedance 2.5 的 edit/extend **仍未真跑端到端**

`ratio=adaptive` + `duration=-1` 已确认火山接受，普通文生视频两服都真跑成功了，
但**「视频编辑」「视频延长」两个模式没真出过片**。测试服上跑一次编辑 + 一次延长
（传参考视频 + 提示词写触发词），确认能出片、语义对。顺带验越界文案是大白话那两句。

## 🎯 待办 2：「失败趋势（近 30 天）」要等数据长出来

保留期已从 7 天改成 31 天（`scripts/cleanup-old-data.mjs` 的 `EVENT_RETENTION_DAYS`），
但**正式服已被删的历史找不回来** → 大约 3 周后那张图才会真正长满 30 天。
⛔ 在此期间别把「柱子还是不满 30 天」当成没修好；判据是图能渲染 30 根柱 + 图例写着「保留 31 天」。

## 🎯 待办 3：正式服 3 条历史 console error（低优先，非本批引入）

老资产 poster 地址存成 `https://localhost:3000/generated/...` → `ERR_SSL_PROTOCOL_ERROR`。
是很早期上传视频封面留下的脏数据。要清就写个一次性脚本把 `MediaAsset` 里带 `localhost:3000` 的
url 前缀替换成相对路径，⛔ 动前先 dry-run 数条数。

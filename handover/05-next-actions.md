...
## ✅ 当前状态（2026-08-09 第六十次会话末）：**本地 = 测试服 = 正式服 = GitHub 全部 `v1.0.0.94`**

| | 版本 / 状态 |
|---|---|
| 本地 / 测试服 / 正式服 / GitHub | **`v1.0.0.94`** —— **四方同步**；`src/` 逐文件 md5 三方完全一致（194 文件、`1f041835ac49a5ef5db4f1796934e485`）|
| 未提交 | 无（commit `6839819` 已 push）|

⭐ 本次 = **全面审查 v91~v94 这一批 + 测试服真走界面验证 + 整批上正式服**。
**静态审查和真机验证都没发现问题，一行代码都没改。** 细节在 `CHANGELOG_2.md` 顶条（六节）。

已验到的关键判据（⛔ 别重复做）：
- 4 个落点 × 5 件事全过（粘 20000 字一个不丢 / 计数器变红 / 按钮灰 / 黑底提示 / 输入卡高度 136px 不变）；
- 后台「上传规则」**只保存一半不清空另一半**（连续 3 次只发 prompt，upload 那份三次都在）；
- 后台改值 → `/api/model-availability` → 前台生效（2.5 设 9999 真机看到 `2 / 9999`），测完已改回 `{}`；
- 服务端「只记日志不拦」真落盘一行 `prompt-length-over-limit`（used 23 / maxLength 10）；
- 正式服 `/api/announcement` + `/api/auth/me` **有 no-store**，`/api/media-thumbnail` **没被加**（白名单生效）；
- 正式服后台：2.0 系 9/3/3 + 文字 3500、2.5 30/10/10 + 文字 14500；三个 2.5 开关 = 开 / **关** / 开；
- 两服都真跑过一次生图成功；console 全程 0 error。

⚠️ **要提醒用户强刷一次（Ctrl+F5）** —— 他链路上那份被透明代理缓存的旧公告副本可能还没过期。

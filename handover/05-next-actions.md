...
### 🎯 待办 1：M031 工作流节点传参考图偶发"静默挂不上"（⛔ 根因未知，**严谨复现之前不许动代码**）

完整定义（现象 / 两个已被证伪的归因 / B 的假设 / 复现设计）在 **`06-memo-tasks.md` 的 M031**。
...
- 双 AI 协作频道：`handover/09-ai-review-channel.md`（临时机制，用户随时可删）。
- 新增 2 条铁律在 `AGENTS.md`（Next 配置名先读 docs / matcher 要测编译产物正则）。
- `03-deploy-and-servers.md` 新增 2 条（本机 curl 会话对 Host 敏感 / 测试服 nginx 是 3 份）。

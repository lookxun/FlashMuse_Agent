...
## ✅ 当前状态（2026-08-11 第六十四次会话末）：**本地 = 测试服 = 正式服 = GitHub 全部 `v1.0.0.98`**
...
| 迁移 / 基础设施 | ⭐ **1 个 Prisma 迁移**（`20260811010000_content_moderation_term_sort_order`，两服 Applying）；无 compose/nginx 改动 |
| 自查 | `tsc` 0 |
| 回滚点 | `/opt/flashmuse/app-backups/20260811-180453-presync-v1.0.0.98`（145M）+ 正式库备份 `pre-deploy-v98` |

⭐ 本次 = **修内容审核词库的「王丹」乱码 + 词库排列三端不一致**，两服都已上线。
详细过程（**十节**，含根因的模拟实验、**第七节「恢复资产」**、第八节取证细节）在 `CHANGELOG_3.md` 顶条（第六十四次会话）。

- ✅ **乱码根因已坐实**：词表分段传入浏览器、每段各自 UTF-8 解码，`丹` 的 3 字节被切在第 4587 字节
  （3 字节 / base64 对齐边界）→ `王` + 2 个 U+FFFD。**服务端无辜**；正式服那份是 `\copy` 照搬过去的。
- ✅ **顺带解除真伤害**：坏行的 `normalized` 被剥成「王」一个字 → 测试服上任何含「王」的提示词都被拦。已实测解除。
- ✅ **排序根因已修**：新增 `sortOrder` 列（原来 586 行 `createdAt` 完全相同、`ORDER BY createdAt` 无决定性依据）。
- ✅ **三端数据统一成权威 587 词**（原 586 + 用户加的 `毛主席`）。
  ⚠️⚠️ **以后核对词库数量以 587 为准**，历史文档里的 586 已过期。

### ⛔ 本次故意没做 / 不用再做的事（别重复劳动）

1. ⛔ **没去追"当初到底是哪段代码把词表分段传进浏览器的"** —— 那是 2026-08-07 一次性的人工填库操作
   （Playwright 填后台文本框），**不是产品代码里的链路**，现在也不会再走一遍。根因已坐实到"解码边界"这一层，
   够用了。⭐ 真要防，防线是"以后填库别分段"，而不是改产品代码。
2. ⛔ **没给 `/admin/api/content-moderation` 加"拒收含 U+FFFD 的词"的校验** ——
   想过，但没做：① 用户可能有正当理由录入奇怪字符 ② 真正的问题是"进来之前就坏了"，
   在入口拦只会变成静默丢词。🗳️ **如果要加，先问用户**（我倾向"拦下来并明确报错告诉管理员第几个词有问题"）。
   ⭐ 现成的巡检判据（零成本、随时可跑）：
   `SELECT count(*) FROM "ContentModerationTerm" WHERE "value" LIKE '%'||chr(65533)||'%'` 必须为 0；
   `SELECT count(*) FROM "ContentModerationTerm" WHERE char_length(normalized)=1` 也必须为 0
   （**归一化后只剩 1 个字的匹配词几乎一定是脏数据，且伤害面极大**）。
3. ⛔ **没有清理 `.runtime/` 里的恢复资产**（`canonical-terms.json/txt`、`gen-fix.mjs`、`fix-terms.sql`）——
   **故意留着**，下次再核对词库直接用。`.runtime/` 是 gitignored，只在本机。
4. ⚠️ **没有验证"再点一次保存规则后顺序还对不对"** —— 逻辑上必对（新代码按下标写 `sortOrder`），
   但**没在界面上真点过「保存规则」**（怕碰那 587 词）。⭐ 下次谁在后台正常改词库时，**顺手确认一下顺序没乱**即可。
5. ⚠️ **老数据的 `sortOrder` 目前全是 0**（迁移的默认值），顺序靠第二级 `createdAt` 兜底（已被我改成递增、distinct=587）。
   ⛔ **别以为 sortOrder 全 0 是没生效**；下次管理员在后台保存一次，就会被写成 0..586 的真实下标。

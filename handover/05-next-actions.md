...
## ✅✅ 当前状态（2026-08-08 第四十八次会话末）：**四方同步 `v1.0.0.81`；内容审核全套已上正式服 + 586 词已同步生效；⛔ 无紧急待办**

| | 版本 / 状态 |
|---|---|
| 本地 | `v1.0.0.81`，`tsc` exit 0，工作区已 commit |
| 测试服 | `v1.0.0.81`（3 迁移已应用；586 词 + 开关开）|
| 正式服 | `v1.0.0.81`（四域名 200；3 迁移已应用；**586 词已同步 + 开关已开**）|
| GitHub | `v1.0.0.81` |

⭐ 本次细节全在 **`CHANGELOG_2.md` 顶条（第四十八次会话）**。
⛔ **没有紧急待办**：用户要求的「测试服 → 测试 → 正式服 → 同步敏感词 → 推 GitHub」全部做完并验通。

### ✅ 本次会话已完成（不用再做）

1. **v81 两服部署完成**（含 3 个 Prisma 迁移），正式服备份 `20260808-072915-presync-v1.0.0.81`。
2. **正式服敏感词同步**：0 → **586 词**、开关 false → **true**（纯 INSERT，零删除，全程服务器内搬运）。
3. **正式服端到端验通**：命中 → 400 + `CONTENT_POLICY_BLOCKED` + 干净中文、**不带 B_xxx**、**0 扣费**；
   正常提示词不误拦；真跑生图成功（−3 积分）；语义审核 3 条全处理（flagged 1 / clear 2）、不拦用户。
4. ⭐ **把第 46/47 次会话一直挂着的「后台页没人亲眼验过」全部验完**（8 项，见 `01-current-status.md`）。
5. **巡检 6 项全过**、正常浏览 0 console error。
6. **commit + push GitHub**，四方同步恢复。

### 🎯 待办 1：M040 —— 把「红字幂等」固化成 `npm test` 用例（第 44 次立项，仍没做）

- 现在的防线是 `error-message.ts` 第 217 行那份**白名单式幂等保护**，**每加一句新红字都得记得往里加一条，漏了就串**。
- ⭐ **最低成本做法**：固化成一个用例 ——「**任意一条映射结果连跑 3 遍必须完全一致**」，
  清单抄 `CHANGELOG_2.md` 第 44 次会话第二节的 **37 条 + 8 条反向**。现在 `npm test` 只 15 个用例、1.2 秒，成本几乎为零。
- ⭐ 内容审核那句拦截红字**已实跑验证过不需要进白名单**（不含会被兜底抢走的关键词），但仍该被测试兜住。

### 🎯 待办 2：内容审核剩下的已知取舍（都不是 bug，改前问用户）

1. ⚠️ **每次生成都会送一次语义审核**（未命中词库时）→ 每次生图/生视频多一次文本模型调用。
   ⭐ 想省钱只能做**抽样**（按比例/按用户/按来源）或只审高风险来源，**改前必须问用户**。
2. ⚠️ **工作流节点重试没有去重**（对话流失败卡已用 `suppressContentModerationRecord` 去重）。
   要统一就照对话流那条路把标记传进 `workflow-tldraw-canvas-inner.tsx` 的两处 fetch。
3. ⚠️ **`ContentModerationEvent` 保留 30 天**，后台没有"手动清理/忽略"入口。
4. ⚠️ **审核只覆盖生成提示词**（`/api/image`、`/api/video`）。⛔ 不管普通聊天/通用模式/Agent 对话/提示词优化/图片反推 —— 用户当初定的范围，要扩再问。
5. ⚠️ `initialHidden`（DB `termsHidden`）是**半废字段**（进页面恒隐藏，不用它做初始显示），是否清理待用户定。
6. ⚠️ 锁是**页面级 UX + 共享状态**，服务端 mutation 仍只靠管理员 cookie，没按 `editUnlocked` 硬拦。

### ⚠️ 待办 3：排查红字时请跳过 B_193 / B_194（我的测试造成的）

本次为验证内容审核，我手打 `/api/image` 时**两次写错 model id**（`byteplus:seedream-4-5`，正确是
`byteplus:conversation-image.seedream-4-5`）→ 正式服留下 **B_193 / B_194** 两条红字。
⛔ **不是用户问题、不是 bug、不用归档**（它们本来就落在兜底桶，但根因已知）。

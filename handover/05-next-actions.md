...
## ✅ 当前状态（2026-08-09 第六十一次会话末）：**本地 = 测试服 `v1.0.0.95`；正式服 / GitHub `v1.0.0.94`**

| | 版本 / 状态 |
|---|---|
| 本地 = 测试服 | **`v1.0.0.95`** —— 已部署并真走界面验过、巡检 6 项全过 |
| 正式服 / GitHub | `v1.0.0.94` —— **v95 还没上正式服**（等用户拍板）|
| ⚠️ 未提交 | 本批全部改动（3 个源文件 + 1 个新测试 + 交接文档）**还没 commit** |
...
1. **修掉「公告『新增』显示成『新建』」** —— 影响所有简体用户（默认）。
   ...
   ⭐ 修法：简体分支**只还原存下来的原文，没存过就原样不动**（属性同理）+ **删掉**那个有损反向函数与两张反向表。
...
**已验到的关键判据（⛔ 别重复做）**：
- ⭐⭐ **确定性判据**：切繁体 → 再切回简体（**旧代码走这条路必错**）→ 公告仍「新增」且完整还原（影片→视频），全页面 0 处「新建」；
- 连刷 6 次 + 单页 1~8 秒逐秒采样：横幅稳定 231 字、「新增」恒在、「新建」**从未出现**；
- ⭐ 铁证：侧边栏「**新建**工作流」（我们自己的界面文案）保持不动 → 说明"该动的没动、不该动的也没动"；
- 巡检 6 项：登录 / 对话模式 / 工作流点节点不崩 / 资产库 / **真跑生图成功** / 后台 `/admin` 正常，**console 全程 0 error**；
- 构建产物验证：`/api/health` = v95、`x-app-version` = v95、页面显示 v1.0.0.95、`No pending migrations`。

⚠️ **踩到的坑（下次省时间）**：① 生产构建**会压缩局部函数名** → 拿函数名 grep `.next` 产物**验不了**（三个名字全 0，包括本该存在的那个）；
② **PowerShell 重定向 `> file` 写 UTF-16LE** → node 读出来是乱码、正则全不匹配（要用 `execSync` 在 node 里直接取 diff）。

🎯🎯 **下一个 AI 的最优先任务：仍然是「间断性卡死」bug 的静态定位**（见下方待办，⛔ 只许加日志不许改行为）。
🗳️ **等用户拍板**：v95 要不要上正式服。
...
## 🎯 待办 3：搭车清掉 `MAX_DRAFT_INPUT_LENGTH` 这个死常量

`src/lib/chat/chat-workbench-core.tsx:966` 的 `export const MAX_DRAFT_INPUT_LENGTH = 2000` **已经没人用了**
（唯一引用是 `chat-workbench.tsx:789` 的一句注释）。
⛔ 它是个坑：下一个人可能又拿它去 `slice` = 把"超字数不删字"这条口径破坏掉。
⭐ **不为它单独 bump 一版**，等下次有别的改动时搭车删掉。

## 🎯 待办 4：`CHANGELOG_2.md` 已到 **2388 行 / 228KB**，快到轮转阈值了

- 轮转规则（`00-README.md`「交接文档维护规则」）：**≈400KB 或 2000+ 行**就该新建 `CHANGELOG_3.md`。
  ⭐ 行数已超（2388），体积还没到（228KB / 400KB）→ **下一次会话末如果又长一大截，就开卷 3**。
- ⭐ 开新卷的固定 5 个动作在 `00-README.md` 里写着，照做即可（⛔ 别忘第 5 步：更新 `00-README` 的文档索引两行）。
- ⚠️ 顺带：卷 2 的 **887~1033 行是编码受损原文**（760 个 `U+FFFD`，第 50~53 次会话）。

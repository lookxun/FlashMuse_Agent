...
1. 🗳️ **语音：等你拍板要不要换掉新对话的默认模型。**
    ⚠️ 默认仍是**已下架**的 `fish-audio/s2.1-pro-free`（菜单里显示 Fish Audio S2.1 Pro）→
    用户不手动换模型就必然红字 `B_498/B_500`（本批正式服冒烟就得先手动切到 Qwen 才跑得通）。
    选项：① `DEFAULT_AUDIO_MODEL` 改 `qwen/qwen-audio-3.0-tts-plus` ② 把 fish 两个从菜单藏掉 ③ 不动。
2. ⭐ **画质增强极速（海外 BytePlus）到现在一次都没真跑过**（代码与国内那条同一份、key 已配、后台显示已启用）。
    要验就在**测试服**选一次「画质增强极速 → 高清720p」，对账 `usd = 秒/60 × 系数 × 0.1033`。
3. ⭐ **深度捕捉还有两条"没真跑过"的组合**：① **>25 秒的源视频**（验收费与显示都按 25 秒截断、成品真是 25 秒）

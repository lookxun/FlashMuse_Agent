...
> **⭐⭐ 最新（2026-07-21 部署 session）：✅ 已部署正式服，四方同步【正式服=测试服=本地=GitHub v1.0.0.36 / commit `dd37a78`】，四域名 200，无遗留待推、无本对话新增 Prisma 迁移。** 本对话按用户指令：① **先把上一 session 积压的 v1.0.0.34 整份部署上正式服**（rsync staging→prod、entrypoint 自动 apply 迁移 `20260721000000_media_asset_duration_float`、正式服 DB 跑 `backfill-prompt-mentions.js`=fixed0/ok84/skip3、commit `5bb0fc2`）；② **@引用资产弹窗左侧分类"滚动条常驻"**（v35，共享组件 `asset-mention-picker.tsx`，`overflow-y-auto`+自定义非叠加滚动条，溢出常显、不加高弹窗，三处 @引用资产都覆盖）；③ **修「@引用资产同一上传视频/资产显示成两个」**（v36，根因=`getAssetIdentityKey` 原 mediaId 优先，同一文件"消息内嵌引用(无mediaId)"与"资产库权威记录(有mediaId)"两份 key 不同漏判 → 改成 **url 优先**，url 才是文件唯一身份；对话流/工作流/资产库三处共用同一 `assets`+函数=一处改全覆盖所有分类）；④ **再次部署正式服 v1.0.0.36 + push `dd37a78`**。**下一个 AI：无遗留待办；非紧急历史项见 05 顶条。** 详见 CHANGELOG / 01-current-status / 05-next-actions 顶条。

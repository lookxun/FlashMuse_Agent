...
### 2026-06-20 线上追加：媒体新表运行时重复、移动分类写回、排序和前端去重修复

- 修复运行时远程/本地重复媒体。新增 `src/lib/media-assets.ts`，统一 `MediaAsset` URL 归一和 saved job canonical 合并；当 `.runtime/media-save-jobs.json` 显示远程 URL 已保存成本地 `/generated/...` 时，保留本地正式媒体，合并远程媒体的成本、流水、参数、提示词等字段，远程媒体归档隐藏为 `duplicate_remote_url`。`/api/media-save-status`、`/api/media-assets POST`、`upsertWorkspaceMessages()` 均接入该逻辑，防止后续新生成图片/视频再次出现远程/本地双记录。
- 修复资产库用户状态写回。`src/app/api/media-assets/route.ts` 新增 `PATCH`，支持移动分类、重命名、软删除、恢复，写入 `UserAssetState.currentCategory/currentName/deletedAt/purgeAt/restoredAt/sortOrder`。`src/components/chat-workbench.tsx` 的移动分类、重命名、删除、恢复均已调用该接口，不再只改旧前端 `assets`/workspace JSON。
- 修复移动分类和整体排序。移动分类时写时间戳级 `sortOrder`，新移动资产能排在目标分类首位；旧迁移遗留的小 `sortOrder` 不再干扰排序，普通资产按 `MediaAsset.firstSeenAt desc / createdAt desc / id desc` 最新优先。修正后虚线生成卡仍固定第一个，最新图片在其右侧。
- 修复前端分类切换导致同一图越切越多。前端资产合并现在按 `mediaId -> normalize(url) -> id` 做全局去重；分类第一页加载按服务端返回顺序替换旧分类数据，并移除内存中同一媒体旧副本；分页加载更多只追加未出现过的资产；移动分类同样会去掉旧副本。
- 线上 `ID_636611` 验证：部署前 `visibleDuplicateGroups=6`，含 4 组图片和 2 组视频远程/本地重复；执行 `node scripts/migrate-user-media-assets.mjs --user=ID_636611 --apply` 后复审 `visibleDuplicateGroups=0`、`visibleDuplicateItemsExtra=0`，成本审计 `unmatchedLedgers=0`、`partialLedgers=0`。分类移动接口测试显示移动到角色图片后 `character_image 31 -> 32`、`conversation_images 154 -> 153`，恢复后回原值。角色图片接口前 5 个为 `角色33 / 角色31 / 角色30 / 角色29 / image_9_d22`，时间递减，说明最新优先。
- 本轮部署到马来：使用 `E:\project\【2】server\马来西亚服务器\ByteplusVPS.pem` 登录 `root@101.47.19.109`，线上项目 `/var/www/flashmuse`，执行 `/usr/local/bin/deploy-flashmuse-production.sh`。线上备份目录包括 `20260620194758-media-new-table-fix`、`20260620195515-media-move-sort-fix`、`20260620195952-media-move-front-ui-fix`、`20260620200330-asset-order-fix`、`20260620201200-asset-order-server-fix`、`20260620202220-asset-dedupe-ui-fix`。所有 build 通过，仅既有 Turbopack/NFT warning，PM2 online，阿里 `_next/static` 已同步。当前未提交 GitHub。

### 2026-06-20 线上追加：新表读写闭环、后台口径、历史排序、审核和 @ 引用修复

- 后台改为新表口径。生成记录首屏、用户展开详情、媒体列表、资产库列表和概览统计优先读 `MediaAsset + UserAssetState`；`WorkspaceSession/WorkspaceMessage` 只用于对话结构，`CreditLedger` 用于扣费。修复视频封面或旧流水类型导致的视频同时出现在“对话流图片”的问题。
- 新增 `src/app/api/media-assets/route.ts`。资产库生成/上传成功后直接 upsert `MediaAsset + UserAssetState`，避免新媒体只存在前端或旧 workspace JSON 里。`src/lib/workspace-sessions.ts` 保存 `WorkspaceMessage` 后会同步对话流生成图片、视频和上传图到新表。
- 对 `ID_636611` 重新执行 `node scripts/migrate-user-media-assets.mjs --user=ID_636611 --apply`，补回用户刚才生成后消失的新媒体。当前可见媒体 `271`，可见分类 `character_image=27 / scene_image=10 / shot_image=19 / conversation_images=150 / conversation_videos=65`，可见重复 `0`。
- 历史对话排序改为真实内容时间：优先按最新 `WorkspaceMessage.createdAt`，没有消息才按 `WorkspaceSession.updatedAt`，最后按 `sessionId`。解决“在吗”等旧对话因 `updatedAt` 被保存/迁移刷新而排到前面的问题。
- 资产库入口和分类切换不再强制刷新。前端新增 `loadedAssetFilters`，已加载分类直接使用内存缓存；滚动到底仍按分页继续加载。`@` 资产弹窗会自动补拉可引用的 `character_image / scene_image / shot_image / conversation_uploads`，并将加载态改为居中蓝色 spinner + `加载中...`。
- BytePlus 自动审核修复：审核失败也写回 `UserAssetState` 和旧 workspace，后续同图不会无限重复审核；按用户要求最多自动重审 3 次，次数存在内部错误前缀且前端不展示。同一视频消息的“系统检测到真人图片...”系统提示只出现一次。
- 错误文案拆分：参考图素材审核版权拒绝显示 `审核图片可能涉及版权限制，平台已拒绝，请换其它图片重试。`；输出视频轮询阶段版权风控仍显示 `输出视频可能涉及版权限制，平台拒绝生成。重新生成有可能会成功。`。B116 已确认是输出视频版权风控，B111 是审核图片版权拒绝。
- 本轮上述修复均已部署线上，build 通过，PM2 online，阿里静态已同步；未提交 GitHub。

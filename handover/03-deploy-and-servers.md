...
- ⭐⭐ **每部署完一台，必须真上号点一遍看有没有崩（2026-07-29 用户加；2026-08-24 改口径；2026-09-12 再改）**：
  **curl 200 / 版本号头对了 ≠ 没崩**。登录号不变：**三个环境一律 `12424740@qq.com`/`dragonstar`**；后台才用 `lookxun@163.com`，只看页面、别生成、⛔ 正式服公告别动。
  ...
  | ⛔⛔ 禁用 | `fish-audio/s2.1-pro-free`（那个"免费"的） | — | **OpenRouter 已全系下架 fish-audio**，点了必得 `B_498/B_500`，**证明不了"没崩"** |
  | ⛔ 禁用 | GPT / Gemini / Recraft Pro 等十几积分的 | — | 不是"最便宜" |

  - ⚠️⚠️ **新对话的默认语音模型就是那个已下架的 fish 免费版** → **必须先手动把模型切到 Qwen** 再发，
    否则你测到的是一个必然失败的模型（2026-09-12 实操时就是这么切的）。
  - ⭐ **判据三条，缺一不可**：
    ...
    ⛔ 别拿"页面能打开 / 没报错"当通过。
  - ⭐ 建议**新建一条对话**跑（标题带版本号，如 `v1.0.1.25 正式服冒烟：一切正常。`），方便写进交接文档的「留痕」节。
  ...
  - 🔴 **原先的判据是"用免费语音 `fish-audio/s2.1-pro-free` 冒烟"，2026-09-10 失效** ——
    **OpenRouter 把 fish-audio 全系下架**（`GET /api/v1/models` 里一个 `fish` 都没有，只剩
    `openai/gpt-audio` / `openai/gpt-audio-mini`；直打 `fish-audio/s2.1-pro-free` 返回
    `404 No endpoints found`，两服一致 → 供应商问题，不是我们的代码）。
  - ✅ **2026-09-12 坐实：挂的只是 fish-audio 那两个，`qwen/qwen-audio-3.0-tts-plus` 和
    `minimax/speech-2.8-hd` 都还活着**（正式服真出了 4 秒音频）→ 所以冒烟改用 Qwen，语音功能**并没有全站挂掉**。

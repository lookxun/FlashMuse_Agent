...
- ⭐⭐ **每部署完一台，必须真上号点一遍看有没有崩（2026-07-29 用户加；2026-08-24 改口径；2026-09-12 再改）**：
  **curl 200 / 版本号头对了 ≠ 没崩**。登录号不变：**三个环境一律 `12424740@qq.com`/`dragonstar`**；后台才用 `lookxun@163.com`，只看页面、别生成、⛔ 正式服公告别动。
  ...
  ### 🗣️⭐⭐⭐ 2026-09-12 用户拍板：正式服冒烟 = 跑一条最便宜的生成（覆盖上面第 2 条）

  > 用户原话：「**以后推正式服后 就用最便宜语音或图片模型生一条，保证正式服不崩即可。**」

  | | 用什么 | 成本 | 备注 |
  |---|---|---|---|
  | ✅ **首选** | 语音 **Qwen Audio 3.0 TTS Plus**（`qwen/qwen-audio-3.0-tts-plus`） | **1 积分** | 2026-09-12 实测 `usd 0.00042` → 全平台最便宜的一条真实生成 |
  | ✅ **同样可用** | 语音 **Fish Audio S2.1 Pro（收费版，`fish-audio/s2.1-pro`）** | **1 积分** | ⭐⭐ 2026-09-12 **用户澄清：下架的只是"免费版"，这个收费版没问题、可以拿来冒烟**；`usdPerChar 0.000015`，一句话同样只扣 1 积分。**它就是现在菜单里默认选中的那个**（免费版已被后台关掉，菜单里看不到） |
  | ✅ 备选 | 图片 **Seedream 5.0 Lite**（`byteplus:conversation-image.seedream-5-0`） | 2 积分/张 | **只生 1 张**。语音整条链路都挂了才用它 |
  | ⛔⛔ 禁用 | `fish-audio/s2.1-pro-free`（那个**免费版**，菜单名「Fish Audio S2.1 Pro（免费）」） | — | **OpenRouter 只下架了它**，点了必得 `B_498/B_500`；⭐ 但**后台开关已经把它关掉了**，菜单里压根不出现 → 正常操作已经撞不到 |
  | ⛔ 禁用 | GPT / Gemini / Recraft Pro 等十几积分的 | — | 不是"最便宜" |

  - ⭐⭐ **2026-09-12 用户澄清（把上一批写错的结论纠正过来，⛔ 别再传错）**：
    🗣️ 原话：「**本来有两个 fish 一个是免费的一个是收费的。openrouter 下架的是免费的那个。我们平台后台我关掉了。
    所以你现在首选的那个 fish 是收费的。他是没问题的。你可以用这个收费的 fish 跑冒烟。**」
    → ⛔ **「fish-audio 全系下架 / 新对话默认模型必红字」这两句话是错的**（上一批的结论），
    ✅ 事实是：**只有免费版 `fish-audio/s2.1-pro-free` 被下架，且它已被后台开关关掉**；
    **收费版 `fish-audio/s2.1-pro` 活着，而且它就是现在菜单里的默认项** → **新对话直接发就能冒烟，不用手动切模型**。
    ⭐ 代码层留个心眼：`DEFAULT_AUDIO_MODEL = audioGenerationModels[0].id` 仍然指向**免费版那个 id**，
    菜单里之所以落在收费版，是因为**后台开关把免费版过滤掉了** → ⛔ 谁要是在后台把免费版重新打开，红字就会回来。
  - ⭐ **判据三条，缺一不可**：
    ① 界面上真出了**音频卡（带 `00:00 / 00:04` 这种时长）**或真出了图；
    ② **console 0 error**；
    ③ 回库对账 —— `CreditLedger` 最新一行就是这次的，例如
       `09-12 11:25:09 | audio | 语音生成 | qwen/qwen-audio-3.0-tts-plus | credits 1 | usd 0.00042`。
    ⛔ 别拿"页面能打开 / 没报错"当通过。
  - ⭐ 建议**新建一条对话**跑（标题带版本号，如 `v1.0.1.29 正式服冒烟：一切正常。`），方便写进交接文档的「留痕」节。
  - ⭐ 顺手再看三条（都是零成本）：未过期 `GenerationReservation` = 0、
    `grep -c generation-quota-gate-failed` = 0、app 日志尾部有 `[generation-worker] started`。

  ### ⏪ 历史背景（2026-09-10 那次为什么失效了，留档）

  - 🔴 **原先的判据是"用免费语音 `fish-audio/s2.1-pro-free` 冒烟"，2026-09-10 失效** ——
    **OpenRouter 下架了它**（`GET /api/v1/models` 里一个 `fish` 都没有，只剩
    `openai/gpt-audio` / `openai/gpt-audio-mini`；直打 `fish-audio/s2.1-pro-free` 返回
    `404 No endpoints found`，两服一致 → 供应商问题，不是我们的代码）。
  - ⛔⛔ **2026-09-10 那次由此推出的「fish-audio 全系下架、付费版也不可用」是错的**
    （`/api/v1/models` 列表里查不到 ≠ 该模型不可用），**2026-09-12 用户直接澄清：收费版一直好着**。
    ⭐ 教训：**"列表里没有"只能证明列表里没有** —— 要断言某个模型不可用，得**拿那个确切的 model id 直打一次**
    （免费版直打确实 404，而收费版我们从来没直打验证过就一起判了死刑）。
  - ✅ **2026-09-12 坐实：`qwen/qwen-audio-3.0-tts-plus` 和 `minimax/speech-2.8-hd` 都还活着**
    （正式服真出了 4 秒音频），**加上收费版 Fish**，语音功能**从来没有全站挂掉**。
  - ⭐ **另外那几项零成本冒烟仍然有用**（挑与本批改动相关的）：
...
> ⛔⛔ **2026-08-09（第五十八次会话）留下的坑（已在 v94 修掉，留档备查）**：
> 那次为了赶紧修正式服公告的缓存 bug，**只把 `src/proxy.ts` + `src/lib/app-version.ts` 打包推了测试服 v93**，
> 而本地在那之后又叠了一批改动（按模型字数 + 图标收敛）→ **本地 v93 ≠ 测试服 v93**，
> 违反了本项目「版本号一样 = 代码一样」这条核心约定。
> ⭐ **下次部署一律先 `node scripts/bump-version.mjs`（→ v94）再推完整源码**，⛔ 别再往 v93 上叠。
> ⭐ **教训**：只推部分文件的"急救式部署"会破坏版本号的判据 —— 要么整批推，要么推完立刻在交接文档里
> 用最显眼的方式写明"这个版本号只包含哪一半"。

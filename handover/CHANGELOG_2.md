...
>
> **结果：测试服 = 正式服 = 本地 = `v1.0.0.76`；GitHub 仍 `v1.0.0.74`（没让 commit 就没 commit）。**
> **改动只有一个文件 `src/lib/error-message.ts`（+24 行，大部分是注释）。**
...
- ⛔ **GitHub 仍 `v1.0.0.74`**，本地攒了 **v75（4 条日志）+ v76（本次修复）两批**未 commit/未 push。
- ⛔ **红字一条都没归档**：B_123 那类「参考素材没过审」属于**该一直亮着**的（用户侧问题，见
  `07-red-error-triage-and-archive.md` 第十九节 C 小节）。⚠️ 但注意：**历史上被串成"拒绝出图"的那些条，
  在后台是记在「模型拒绝」那一行里的**（`failureReason` 存的是客户端映射后的文案）——
  修复只对**新发生的**生效，⛔ 老数据没回填（也不建议回填，见待办）。

# Handover Changelog · 卷 2（2026-08-03~2026-08-09）· ⛔ 已归档只读，勿改

> ⛔⛔ **本卷已归档（2026-08-09 第六十一次会话末轮转，2388 行超过 2000 行阈值）。新的会话记录写在 `CHANGELOG_3.md`。**
>
> - **当前活跃卷 = `CHANGELOG_3.md`**（2026-08-09 起用）→ 接手先看它顶部的「当前状态摘要」。
> - **卷 1 = `CHANGELOG.md`**（2026-07-21~2026-08-03，约 467KB / 2538 行）同样已归档、只读。
> - 本卷覆盖**第 45~60 次会话**：Seedance 2.5 上线与后台补齐、提示词「超字数不删字」全套、
>   API 统一 `no-store`（修公告被透明代理缓存）、腾讯↔阿里并发分片传输、多处重复实现收敛等。
>   **需要那段区间的详细过程时才翻这里。**
> - ⚠️⚠️ **本卷 887~1033 行是编码受损原文**（用 PowerShell 写中文文件的失误，760 个 `U+FFFD`，第 50~53 次会话）。
...
## 第六十次会话（2026-08-09）：全面审查 v91~v94 这一批 + 测试服真走界面验证 + **整批上正式服 `v1.0.0.94`**（无回归、无需修复）

> | | 版本 / 状态 |
> |---|---|
> | 本地 = 测试服 = **正式服** = GitHub | **`v1.0.0.94`** —— 四方同步，`src/` 逐文件 md5 **三方完全一致**（194 文件，hash `1f041835ac49a5ef5db4f1796934e485`）|
>
> ⭐ 用户指令：「全面查一下有没有问题，会不会影响其它功能 → 可以上测试服测 → 有问题就修 → 没问题全部推正式服 → 正式服确认不要崩」。
> ⭐ **结论：静态审查 + 真机验证都没发现问题，一行代码都没改**，直接整批上了正式服。
> 无 Prisma 迁移（两服都是 42 个迁移目录、entrypoint 输出 `No pending migrations`）、无 compose/nginx 改动。

### 一、静态审查（逐文件读 diff，共 15 改 + 5 新，378 增 / 203 删）

重点核了这几个「最容易出回归」的地方，都是干净的：

- ⭐ **`src/proxy.ts` 的 no-store 会不会打掉别人的缓存**：全仓 `grep Cache-Control` 只有 **`/api/media-thumbnail`**
  自己设 `immutable`，而它**正是白名单里那一条** → 不冲突。matcher 排除的 3 个上传路由是 POST，不受影响。
- ⭐ **`getPromptLengthOverrideKey` 的 2.0↔2.5 隔离**：`npx tsx` 真 import 跑 **25 条**（含**双向隔离**：
  给 2.0 系写 override → 2.5 不变；给 2.5 写 → 2.0 系不变；开关关掉 = 回该模型默认值而不是 2000）。
- ⭐ **`MAX_DRAFT_INPUT_LENGTH` 变成死常量**（还导出着 = 2000）：**无功能影响**，但它是个坑
  （下一个人可能又拿它去 slice）→ 记进待办，等下次有别的改动搭车删掉，**不为它单独 bump 一版**。
- ⭐ **`BlackHoverTooltip` 包住 4 个按钮会不会破坏布局**：它渲染的是 `inline-flex` 的 `<span>`，
  `shrink-0` 从按钮挪到了包装层、资产库那个按钮补了 `w-full` → 真机量宽度 339px（仍铺满），没有回归。
- ⭐ **`WorkflowSelectedNodeOverlay` 里新加的 3 个变量是普通表达式、不是 Hook**（那个组件有提前 return，
  加 Hook 会触发 React #310 把整个画布搞崩）→ 真机点节点/开快捷编辑，画布零报错。
- `tsc` 0、`npm test` 15/15、6 个改动/新增文件 eslint 0。

### 二、测试服真走界面（`AGENTS.md` 铁律：只测纯函数/接口不作数）

4 个落点 × 5 件事，**全过**（测试号 `12424740@qq.com`）：

| 落点 | 计数器时机 | 超限变红 | 粘贴不丢字 | 按钮灰掉 | 黑底提示 |
|---|---|---|---|---|---|
| 对话流 | 空=无数字、有内容=`3 / 2000` | `20000 / 14500` 变红 | **粘 20000 字，一个都没丢** | ✅ | 「当前模型提示词只支持14500字！」|
| 资产库生成 | ✅ | `5003 / 2000` | 粘 5000 字全留 | ✅ | ✅ |
| 工作流节点 | ✅（显示**合计**）| `4003 / 3500` | ✅ | ✅ | 「…只支持3500字！」|
| 图片/视频快捷编辑 | ✅ | `4005 / 3500` | ✅（`maxlength` 属性已确认**不存在**）| ✅ | ✅ |

- ⭐ **"不跳高"是量出来的**：输入卡片高度 **空 136px = 有内容 136px**（⛔ 别靠肉眼看）。
- ⭐ **回车兜底真被触发**：对话流「提示词已超过 14500 字（当前 20000 字），请删减后再发送」、
  工作流「输入框和连接文本合计已超过 3500 字（当前 4003 字），请删减后再生成」，且**字一个没被删**。
- ⭐ **换模型上限跟着变**：视频节点 2.0 = 3500 → 切 2.5 = 14500 → 切回 2.0 Mini = **3500**（证明 2.0 系共用一条）；
  对话流切图片生成 = 2000。
- ⭐ **真跑一次生图成功**（测试服 Seedream 4.5，积分 94360 → 94357）；**console 0 error**。

### 三、后台「上传规则」页（本批最大的新 UI）

- 20 行全部有图标、「文字」列排在「文件」**前面**；默认值真机核对：
  **2.0 系 3500 / 2.5 = 14500 / 其余 2000 / 参考模式行显示「跟随上面」**（= 和运行时 `getPromptMaxLength` 一致，
  不会出现"面板显示 2000、实际 14500、管理员一保存就砍到 2000"那种伤害）。
- ⭐⭐ **验了最危险的那条：只保存一半不会清空另一半**。
  连续 3 次只发 `promptLengthOverrides`，`uploadRuleOverrides`（`openai/gpt-5.4-image-2` = 16 张）**三次都原样在**。
- ⭐ **后台改值 → 前台真生效**：把 2.5 设成 9999 → `/api/model-availability` 带出来 → 前台刷新后计数器变 `2 / 9999`。
- ⭐ **服务端「只记日志不拦」也验到了**（做法留档，零额外成本）：
  先把 Seedream 4.5 的上限调成 10（此时前端还是旧缓存 = 2000），再从界面正常发一条 23 字的生图 →
  **没被拦住、图正常出**，而日志里精确落了一行：
  `{"event":"prompt-length-over-limit",...,"extra":{"used":23,"maxLength":10,"over":13,"flow":"conversation","promptMode":"image"}}`。
- ⭐ 测完把测试服 `PROMPT_LENGTH_OVERRIDES` **改回 `{}`** 并复验（`uploadRuleOverrides` 全程未被动）。
- 后台 12 个 tab 逐个打开无崩、无 console error。⛔ **公告 tab 只看不碰**。

### 四、正式服部署（严格按 `03-deploy-and-servers.md` 的 9 步）

1. 备份 `/opt/flashmuse/app-backups/20260809-190927-presync-v90`（145M）；
2. staging→prod 整份 rsync（**不 bump**）→ ⭐ **判据：prod 的 `src/` md5 与 staging 完全相等**（194 文件、同一个 hash）；
3. `up -d --build` → `/api/health` = `v1.0.0.94`；`docker logs` 里 `No pending migrations`；
4. ⭐ 容器内 grep 构建产物，确认本次新字面量真编译进去了（`CACHEABLE_API_PATHS` / `prompt-length-over-limit` /
   `当前模型提示词只支持` 都命中）；
5. `docker cp` 取 `.next/static` → rsync 到阿里**正式**镜像 `flashmuse-static`（腾讯 42 → 阿里 42，**数量一致**）；
6. `/opt/flashmuse/.env` 的 `PUBLISHED_APP_VERSION` 改 v94 + `force-recreate` → 容器内 `[v1.0.0.94]`；
7. 四域名 200；阿里 chunk 抽查 200；
8. commit（`6839819`）+ push GitHub。

### 五、正式服上号巡检（真实用户环境，⛔ 不是 curl）

- ✅ 登录、✅ 对话模式（计数器 `3 / 2000` → 粘 3000 字变红 `3003 / 2000`、字全留、发送键灰、黑底提示）、
  ✅ 工作流画布点节点不崩、✅ 资产库、✅ **真跑一次生图成功**（GPT-5.4 Image 2，积分 9142 → 9127，图正常显示）、
  ✅ 后台 `/admin`（`lookxun@163.com`）；**全程 console 0 error**。
- ⭐ **公告缓存 bug 的判据（本批的核心目的）已在正式服坐实**：
  `curl -sI https://main.venusface.com/api/announcement` → `cache-control: no-store, no-cache, must-revalidate, max-age=0`
  + `pragma: no-cache` + `expires: 0`；`/api/auth/me` 同样有；
  ⭐ 而 `/api/media-thumbnail` **没有被加 no-store**（白名单生效，缩略图 30 天缓存没被打掉）。
- ⭐ 后台「上传规则」正式服核对：**2.0 系那行 9/3/3 + 文字 3500、2.5 那行 30/10/10 + 文字 14500**。
- ⭐ 三个 2.5 开关真机 `aria-pressed` 逐个核对：**视频生成 = true、Agent 自动生成视频 = `false`（故意关着）、
  快捷编辑四选 = true** —— 与设计完全一致。
- ⚠️ **要提醒用户强刷一次（Ctrl+F5）**：他自己链路上那份被透明代理缓存的旧公告副本可能还没过期。

### 六、本次留下的痕迹（⛔ 别当成用户数据）

- 测试服：新建 `工作流_17`（一个空的视频节点，提示词已清空）、对话 `v94巡检：一只白猫坐在木质窗台上晒太阳`、
  `v94服务端字数日志验证：一只灰色兔子在草地上`（这条是**故意**让服务端记一行超限日志用的）。
- 正式服：对话 `v94巡检：一只橘猫在窗台上打哈欠`（1 张图，15 积分）。
- ⛔ 公告在正式服**一次都没动**（禁测铁律）。

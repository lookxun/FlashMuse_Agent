...
| | 状态 |
|---|---|
| 积分充值 | ✅ **已上线**（独立全屏页 `credit-recharge-modal.tsx`）。默认 8 档 ¥50=250 … ¥5000=25000。后台「用户充值 → 积分设置」可改价格/积分（开关锁死；空 env=默认）。「充值积分永久有效」。 |
| 支付 | ✅ **支付宝订单码已通**（测试服 v1.0.1.19，审核已过）。勾协议点充值 → 服务端按下标下单出真码。微信先不做。金额积分必须服务端读设置。应用网关现填测试服 notify。 |
| 支付弹层四态 | ✅ 待支付真码；成功=细线对勾+「支付成功」+「充值成功，获得xxxx积分。」+返回；过期=模糊假码+黄叹号+「二维码已过期请刷新」；拉取失败=同样遮罩+红叹号+「拉取二维码失败请刷新」。刷新=重新下单。 |

...
## ⭐⭐⭐ 付费现状（2026-09-05 定稿，改之前先看这一节）

**只有「积分充值」上线，会员整套停用但代码保留。**

| | 状态 |
|---|---|
| 积分充值 | ✅ **已上线**（独立全屏页 `credit-recharge-modal.tsx`）。8 档 ¥50/100/200/500/1000/2000/3000/5000，一律**基础价 ¥10 = 50 积分**（即 ¥×5）。「充值积分永久有效」。 |
| 支付 | 🟡 **代码已接支付宝订单码**（测试服 v1.0.1.16）。勾协议点充值 → 服务端下单出真码。微信先不做。⛔ 支付宝应用还在审核，现下单会被 `not-online-app` 拒。金额积分必须服务端复算。 |
| 支付弹层四态 | ✅ **本地已改**（114 次，未部署）：待支付真码；成功=细线对勾+「支付成功」+「充值成功，获得xxxx积分。」+返回；过期=模糊假码+黄叹号+「二维码已过期请刷新」；拉取失败=同样遮罩+红叹号+「拉取二维码失败请刷新」。刷新=重新下单。拉码中码框中间「正在生成付款码…」。 |
| 会员订阅 | ⛔ **停用**（`MEMBERSHIP_SYSTEM_ENABLED = false`）。等支付宝连续包月批下来再开。**代码一行没删**。 |
| 会员权益拦截 | ⛔ 全部不生效：模型/画质/并发不拦，人人基础会员，跟停用前的线上行为一致。 |
| 付费协议 | ✅ `/paid-terms`（杭州亿阅科技有限公司），**目前只覆盖积分充值**；协议里明说"后续开通会员订阅将另行公示"。 |
| 后台 | 「用户充值」菜单**保留**（会员设置能看不能改、调会员灰掉、写接口 403）。🗣️ 2026-09-05 用户明确说不用去掉。 |
| 用户中心 | 永远显示基础卡；三列「会员积分 / 充值积分 / 赠送积分」，会员积分恒 0。🗣️ 用户说这一列也不用去掉。 |

⭐ **积分是唯一余额**：`User.credits` 是总余额，`membershipCredits` 只是子标记（会员关着时恒 0）。
⭐ **额度闸门已上线**（对所有人生效，与会员无关）：开工前判「余额 ≥ 已在跑的预估 + 这次的预估」，
不够就拒「积分不足，请充值后再使用模型。」🗣️ 2026-09-05 用户拍板：「**我认，可以这么做，这样不会亏积分，
但是要预估的尽量准**」→ 预估单价已改成用真实扣费数据统计（最大偏差 3%）。

## ⭐ 会员三档（2026-08-28 第九十七次拍板，权威默认值在 `src/lib/membership.ts`）

> ⛔⛔ **下面这套目前全部不生效**（会员停用），只是把口径留着，开关一开就是这套。

后台「用户充值 → 会员设置」可改数字/开关，落到 `.env.local` `MEMBERSHIP_SETTINGS`。没配过 = 下面这套。

**加买积分（永久）**：非会员/标准/高级，¥10 = 50 / 70 / 90 分。
⚠️ **档位 2026-09-04 已改成 8 档 50/100/200/500/1000/2000/3000/5000 元**（`CREDIT_PACKS_CNY`）；
会员停用期间充值页**只显示基础价**，不显示 7 折/6 折。
...
**只卡三项，别的一样：** 图/视频模型、画质、同时生成（并发，0=不限）。对话模型全开。去水印不要。
解除限制/通用模式不绑会员。后台模型开关仍是全站总闸。
⚠️ **首期折 2026-09-04 已改成「按档各一次」**（标准和高级互不影响，某一档月/季/年共用一次）。
⚠️ **买会员 2026-09-03 已改成不补差价、不折天数**（付页面标价；同档续费往后加；低升高马上用高级、低档天数搁着）。
⚠️ **扣费顺序（会员分先扣、永久分后扣）代码已接**（`chargeCredits` 里 `GREATEST(0, membershipCredits - n)`）。

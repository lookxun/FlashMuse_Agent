...
> ## 🆕 2026-09-07（第一百一十二次会话）归档 dry-run 结论：**63 条兜底桶待归，用户拍板「下次上线再归」**
>
> 正式库跑了归档 dry-run（脚本连当前 `DATABASE_URL` 库、读容器内 `.runtime` 诊断日志，**必须在正式服容器
> `flashmuse-flashmuse-app-1:/app` 里跑**）。结论：
>
> - **待排查（`status='failed' && resolvedAt=null`）= 471 条**。绝大多数是明确文案（模型拒绝 208 / 成品被拒 101 /
>   余额不足 32 / 只回文字 30 / 参数不支持 15 / 参考视频版权 11 / 参考素材未过审 9 / 任务失败 2）——
>   **按铁律④修不了、该一直亮着，⛔ 不归档**。
> - **只有两个兜底桶能归，共 63 条**（「服务器繁忙，请稍候再试.....」52 +「请求失败，请稍后再试。」11）。
>   按 requestId 回诊断日志查原文：
>   - **24 条 = Recraft 参考图尺寸超限**（`max image dimension should be no more than 4096` /
>     `min image dimension should be no less than 256`）← v1.0.1.15 已加发送前拦截（归档铁律②）；
>   - 2 条 = gpt-5.4-image-2「请求失败」透传（根因不明，留着）；
>   - **37 条 = 日志已被滚动覆盖、永久不可追溯**（正式服无日志轮转，8/12 前的原文丢了）。
> - 🗣️ **用户拍板「那下次上线再归档吧」**：本批 Recraft 拦截正式服还没上线，现在归档语义超前（线上还没真拦）。
> - **下次上线要做**：① 给 `RESOLVED_RULES` 加一条 Recraft 尺寸规则（匹配 `max/min image dimension`，带 `before`
>   日期下限 = 拦截上线时刻）；② 推正式服 v1.0.1.15；③ dry-run 确认数字后 `--apply` 归掉这 63 条里可归的。
> - ⛔ **脚本坑**：初版分布脚本因 failureReason 内嵌唯一 `(B_xxx)` 前缀 → `groupBy` 每条 count=1 失效，
>   要去前缀 + 截断冒号后内容再归类（临时脚本 `dist-reasons.mjs` / `dist-fallback.mjs`）。

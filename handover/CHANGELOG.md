...
⭐⭐ **正式服那次生视频是本批最有价值的一次验证**：视频跑完的时候我**已经把浏览器标签关掉、会话都掉了**，
而画布里的地址仍然是**本地地址** + 名字 `video_1_w1` + poster 齐全 + `isRunning` 已清
→ 完全证明「服务端在落地那一刻直接改画布」这条路成立（这正是上一批白捡修掉的那个**死链真 bug**：
以前只在用户开着页面时才把远端临时地址换成本地，关了页面就留着会过期的地址）。
...
1. **多标签页那条只验到一半**：两个标签页同时开同一个号，**数据没丢**（提示词条数不变）已验；
   "一个编辑 A、另一个在 B 各自落盘"的完整时序因为工具超时没跑完。按设计它更安全了
   （B 标签页手里 A 是只发标题 → 根本不会回写 A），但没实测。
2. **本地测试残留**：本地库 `工作流_05` 的 canvas 被上面那个 bug 洗成了 2 字节（它原本只有 1 个空文本节点、279 字节），

...
## ✅ 当前状态（2026-08-06 第四十四次会话末：**两服 `v1.0.0.76`；GitHub 仍 `v1.0.0.74`，未 commit**）

- **本地 = 测试服 = 正式服 = `v1.0.0.76`**；**GitHub = `v1.0.0.74`**（🗣️ 没让 commit 就没 commit，本地攒了 **v75 + v76 两批**）。
...
- ⭐ **本次学到一条判据（已写进 `03`）**：**`x-app-version` 在 build 之后仍显示旧版是正常的**
  （它读运行时 `PUBLISHED_APP_VERSION`，故意留到静态同步完才置新版）→ **判"新镜像起来没"要看 `/api/health` 的 `version`**。
- ⚠️ **本次零留痕、零积分消耗**（没上号、没跑生成，用户自己测）。
